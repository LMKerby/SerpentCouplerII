...
/*****************************************************************************/
/*                                                                           */
/* serpent 2 (beta-version) : relerr.c                                       */
/*                                                                           */
/* Created:       2010/11/10 (JLe)                                           */
/* Last modified: 2013/04/17 (JLe)                                           */
/* Version:       2.1.13                                                     */
/*                                                                           */
/* Description: Returns the relative error of scores in statistics           */
/*                                                                           */
...
#define FUNCTION_NAME "RelErr:"

/*****************************************************************************/

double RelErr(long ptr, ...)
{
  ...
  /* Get sum, square sum and number of scores */
  ...
  if (!((X2 >= 0.0) && (X2 < INFTY*INFTY)))
    Die(FUNCTION_NAME, "Invalid X2 %E in %s", X2, 
	GetText(ptr + SCORE_PTR_NAME));
  ...
  Y = (N/(N - 1.0))*(X2/(X*X) - (1.0/N));

  /* Check value */
  
  CheckValue(FUNCTION_NAME, "Y", "", Y, -1E-6, INFTY);
