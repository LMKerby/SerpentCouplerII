...
void WriteTetMeshtoGeo()
{
  ...
  fprintf(out, "Writing tet mesh interface into geometry file \"geo.inp\"...\n");
  ...
  while (loc0 > VALID_PTR)
      ...
      while (loc1 > VALID_PTR)
	{
	  ...
	  fprintf(fp, "\nmat i%ldm%ld %1.5E tms %1.5E\n",
		  (long)RDB[loc0 + IFC_IDX], (long)RDB[loc1 + IFC_TET_MSH_IDX] + 1, 
		  -RDB[mat + MATERIAL_MDENS]*RDB[loc1 + IFC_TET_MSH_DF],
		  RDB[loc1 + IFC_TET_MSH_TMP]);
