...
void SampleMeshDelnu(long id, long np, long idx)
{
  ...
  long precptr, reaptr, lamptr, vplist;
  ...
  for (i = 0; i < MAX_DELNU_RESAMPLE ; i++)
      ...
      for (j = 0; j < MAX_DELNU_RESAMPLE ; j++)
	{
	  ...
	  x = min0 + xbin*(max0 - min0)/(double)n0 + drand48()*(max0 - min0)/(double)n0;
	  y = min1 + ybin*(max1 - min1)/(double)n1 + drand48()*(max1 - min1)/(double)n1;
	  z = min2 + zbin*(max2 - min2)/(double)n2 + drand48()*(max2 - min2)/(double)n2;
