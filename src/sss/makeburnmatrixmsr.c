  ...
  while (iso > VALID_PTR)
    {
      ...
      StoreValuePair(nuc + NUCLIDE_PTR_MATRIX_IDX, (double)mat0, i, id);
      ...
      while (rea > VALID_PTR)
	{
	  ...
	  if ((yld = (long)RDB[rea + REACTION_PTR_FISSY]) > VALID_PTR)
	      ...
	      else
		{
		  ...
		  if ((long)R[1][ii] != nuc)
		    Die(FUNCTION_NAME, "Mismatch in nuclide pointer");
