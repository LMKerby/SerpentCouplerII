...
double FillSTLMesh(long stl, long msh, double x0, double y0, double z0)
{
  long nx, ny, nz, i, j, k, new, ptr, sld;
  double xmin, xmax, ymin, ymax, zmin, zmax, px, py, pz, x, y, z, u, v, w, l, V;
  unsigned long seed;
	  ...
	  if ((new = -(long)RDB[ptr]) > VALID_PTR)
	    {
	      ...
	      V = V + FillSTLMesh(stl, new, x0, y0, z0);
	    }
	  else if (new == 0)
	    {
		...
		Die(FUNCTION_NAME, "Division by zero");
	      ...
	      if (sld > VALID_PTR)
		WDB[ptr + SEARCH_MESH_CELL_CONTENT] = -sld;
