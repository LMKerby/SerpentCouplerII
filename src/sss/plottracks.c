...
/* Last modified: 2016/02/02 (VVa)                                           */
/* Version:       2.1.25                                                     */
/*                                                                           */
/* Description: Plots particle tracks                                        */
/*                                                                           */
/* Comments: - Tätä sallitaan vissiin käytrttävän OpenMP-luupissa, mutta noi */
/*             piirto-operaatiot ei oo thread-safeja.                        */
/*                                                                           */
/*           - Toroidimuunnos puuttuu vielä                                  */
/*                                                                           */
/*****************************************************************************/
...
long PlotTracks(long gpl, gdImagePtr im, long ncol, long nf, double xmin, 
		double xmax, double ymin, double ymax, double zmin, 
		double zmax, long xp, long yp)
{
  ...
  */
	...
	while ((part = FromTrkBank(PARTICLE_TYPE_NEUTRON, id)) > VALID_PTR)
	  {
		...
		if ((type0 != TRACK_END_BC) || (type != TRACK_END_SURF))
		  {
		    /* Draw line (HUOM: tässä tulee ongelma OMP:n kanssa */
		    /* kun useamman threadin piirtämät viivat risteää)   */
  ...
  for (id = 0; id < (long)RDB[DATA_OMP_MAX_THREADS]; id++)
	...
	if (f < 0.0)
	  {
	    ...
	    spd = Speed((long)RDB[part + PARTICLE_TYPE], E);
	...
	DrawTail(gpl, im, ncol, R, idx, xp, yp, xmin, xmax, ymin, ymax, zmin, 
		 zmax);
      }
  ...
  if (1 == 2)
    {
      ...
      /* Draw (HUOM: tässä tulee ongelma OMP:n kanssa kun useamman */
      /* threadin piirtämät pisteet menee päällekkäin)   */
      ...
      if (r > 0.0)
	{
	  ...
	  while (1 != 2)
	      ...
	      if (j == k)
		{
		  ...
		  /* Check if drawn (HUOM: tässä tulee ongelma OMP:n */
		  /* kanssa kun useamman threadin piirtämät pisteet  */
		  /* menee päällekkäin) */
		  ...
		  if (R[3][i - 1] == 1)
			...
			{
			  /* Get color index from overlaps (tässä on joku */
			  /* bugi, piirtää vaaleita viivoja) */
