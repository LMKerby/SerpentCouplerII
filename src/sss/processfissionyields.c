...
/* Version:       2.1.0                                                      */
/*                                                                           */
/* Description: Adds fission yield data to nuclides and perforsm yield       */
/*              cut-off.                                                     */
/*                                                                           */
/* Comments:                                                                 */
/*                                                                           */
...
void ProcessFissionYields(long nuc)
{
	...
	if (yld < 0)
	  {
		...
		if ((long)((long)ACE[yld + FISSION_YIELD_PARENT_ZAI]/10.0)
  ...
  while (rea > 0)
      ...
      if ((mt == 18) || (mt == 19) || (mt == 20) || (mt == 21) || (mt == 38))
	{
	  ...
	  if ((yld = RDB[nuc + NUCLIDE_PTR_NFY_DATA]) > 0)
      ...
      else if (mt - 10000 == 6)
	{
	  ...
	  if ((yld = RDB[nuc + NUCLIDE_PTR_SFY_DATA]) > 0)
	    WDB[rea + REACTION_PTR_FISSY] = (double)yld;
	}
