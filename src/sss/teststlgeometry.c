...
{
  long stl, loc0, ptr, n, m, np, nd, nf, id;
  unsigned long seed;
  ...
  fprintf(out, "Testing STL geometries...\n\n");
  ...
  while (stl > VALID_PTR)
    {
      ...
      fprintf(out, "Testing universe %s by sampling %ld directions in %ld points:\n\n", GetText(stl + STL_PTR_NAME), nd, np);

#ifdef OPEN_MP
#pragma omp parallel private (n, m, id, seed, x, y, z, u, v, w, ptr, loc0)
#endif
      {
...
#pragma omp critical
      ...
      if (nf == 0)
	fprintf(out, "Consistency test passed in all random points.\n\n");
      else
