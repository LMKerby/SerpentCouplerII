...
void ReadRestartFile(long mode)
{
  ...
  double pt, bu, days, adens, mdens, mbu, d0, closest, sum;
      ...
      else
	{
	  ...
	  if (mode == RESTART_REPLACE)
	  ...
	  else
	    {
	      ...
	      if (mat > VALID_PTR) 
		{
		  /* Put densities if not in coefficient calculation */

		  if (RDB[DATA_COEF_CALC_IDX] < 0.0)
		    {
		      WDB[mat + MATERIAL_ADENS] = adens;
