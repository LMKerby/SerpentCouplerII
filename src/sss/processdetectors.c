...
void ProcessDetectors()
{
  ...
  long mt, ne, n, loc0, loc1, loc2, umsh, sflag, idx, i0, phd, m1, m2, fun;
  double sum;
  char str[MAX_STR];
  ...
  while (det > VALID_PTR)
      ...
      if ((long)RDB[det + DET_PARTICLE] == 0)
	{
	  ...
	  if (((long)RDB[DATA_NEUTRON_TRANSPORT_MODE] == YES) &&
	    ...
	    Error(det, "Particle type must be set in combined n/p mode");
      ...
      else
	{
	  ...
	  while (ptr > VALID_PTR)
	      ...
	      else
		{
	      ...
	      /* Switch heat and photon production xs on */
  ...
  while (det > VALID_PTR)
      ...
      if ((ptr = (long)RDB[det + DET_PTR_ADJOINT]) > VALID_PTR)
	{
	  ...
	  m1 = (long)RDB[det + DET_N_RBINS];
	  m2 = (long)RDB[ptr + DET_N_RBINS];
  
	  /* Allocate memory */

	  ptr = NewStat(str, 4, n1, n2, m1, m2);
