...
{
  long nst, ptr, reg, n, cell, lat, loc0, loc1, pbd, umsh, stl, ifc;

  /* Print */

  if (recu == 0)
    fprintf(out, "Counting cells...\n");
  ...
  switch((long)RDB[uni + UNIVERSE_TYPE])
	...
	while ((reg = ListPtr(loc0, n++)) > VALID_PTR)
	  {
	    ...
	    WDB[cell + CELL_VOL_COUNT] = RDB[cell + CELL_VOL_COUNT] + add;
	...
	loc1 = (long)RDB[ifc + IFC_PTR_TET_MSH];
