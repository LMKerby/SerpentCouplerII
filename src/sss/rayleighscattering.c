                        ...
                        long id)
{
  ...
  CheckPointer(FUNCTION_NAME, "(x2d)", DATA_ARRAY, ptr);
  x2d = &RDB[ptr];
  ...
  CheckPointer(FUNCTION_NAME, "(ff2intd)", DATA_ARRAY, ptr);
  ff2intd = &RDB[ptr];
  ...
  if (x2max < x2d[Nd-1]) {
    ...
    if (idx0 == -1)
      Die(FUNCTION_NAME, "Maximum squared form factor smaller than the minimum in the data");

    ff2intmax = ff2intd[idx0] + ff2d[idx0]*x2d[idx0]/(cd[idx0]+1)
                *(exp((cd[idx0]+1)*(lx2max - lx2d[idx0])) - 1.0);

    CheckValue(FUNCTION_NAME, "ff2intmax", "", ff2intmax, ff2intd[idx0], ff2intd[idx0+1]);
  }
  else {
  ...
  do {
    ...
    CheckValue(FUNCTION_NAME, "idx1", "", idx1, 0, idx0);

    x2 = x2d[idx1]*exp(1.0/(cd[idx1]+1)*log((cd[idx1]+1)*(rff2intd - ff2intd[idx1])
                       /(ff2d[idx1]*x2d[idx1]) + 1.0));
