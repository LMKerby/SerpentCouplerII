...
void FissMtxOutput()
{
  ...
  for (n = 0; n < sz; n++)
	...
	{
	  fprintf(fp, "fmtx_t (%3ld, %3ld) = %1.5E ;  ", n + 1, 
		  m + 1, Mean(ptr, 0, n, m));
	  fprintf(fp, "fmtx_t_err (%3ld, %3ld) = %1.5E ;\n", 
		  n + 1, m + 1, RelErr(ptr, 0, n, m));
	}
  ...
  for (n = 0; n < sz; n++)
	...
	{
	  fprintf(fp, "fmtx_p (%3ld, %3ld) = %1.5E ;  ", n + 1, 
		  m + 1, Mean(ptr, 1, n, m));
	  fprintf(fp, "fmtx_p_err (%3ld, %3ld) = %1.5E ;\n", 
		  n + 1, m + 1, RelErr(ptr, 1, n, m));
	}
  ...
  for (n = 0; n < sz; n++)
	...
	{
	  fprintf(fp, "fmtx_d (%3ld, %3ld) = %1.5E ;  ", n + 1, 
		  m + 1, Mean(ptr, 2, n, m));
	  fprintf(fp, "fmtx_d_err (%3ld, %3ld) = %1.5E ;\n", 
		  n + 1, m + 1, RelErr(ptr, 2, n, m));
	}
