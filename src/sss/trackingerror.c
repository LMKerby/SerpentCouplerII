...
void TrackingError(long etyp, double E, long mat, long type, long id)
{
  ...
  double x, y, z, u, v, w, totxs, majorant, dummy;
  ...
  while (lvl0 > VALID_PTR)
    {
      ...
      if ((uni = GetPrivateData(lvl + LVL_PRIV_PTR_UNIV, id)) < VALID_PTR)
	{
      ...
      if ((type = (long)RDB[uni + UNIVERSE_TYPE]) == UNIVERSE_TYPE_NEST)
	{
	  ...
	  ptr = GetPrivateData(lvl + LVL_PRIV_PTR_NEST_REG, id);
      ...
      else if (type == UNIVERSE_TYPE_CELL)
	{
	  ...
	  if ((ptr = GetPrivateData(lvl + LVL_PRIV_PTR_CELL, id)) > VALID_PTR)
      ...
      else if (type == UNIVERSE_TYPE_LATTICE)
	{
	  ...
	  ptr = GetPrivateData(lvl + LVL_PRIV_PTR_LAT, id);
	  CheckPointer(FUNCTION_NAME, "(ptr)", DATA_ARRAY, ptr);
  
	  /* Get region index */

	  if ((idx = TestValuePair(ptr + LAT_COL_CELL_IDX, ncol, id)) > -1)
