...
void Disperse()
{
  ...
  fprintf(outp, "                   5 = cuboid\n");
  fprintf(outp, "                   6 = parallelepiped\n\n");
  ...
  if (type == 1)
  ...
  else if (type == 2)
    {
      ...
      fprintf(outp, "Enter cylinder bottom coordinate (cm): ");

      if (fscanf(fp, "%lf", &zmin) == EOF)
        Error(0, "Missing bottom coordinate");

      fprintf(outp, "Enter cylinder top coordinate (cm): ");
    ...
    {
      fprintf(outp, "Enter length in x-direction (cm): ");

      if (fscanf(fp, "%lf", &xlen) == EOF)
        Error(0, "Missing x-length");

      fprintf(outp, "Enter length in y-direction (cm): ");

      if (fscanf(fp, "%lf", &ylen) == EOF)
        Error(0, "Missing y-length");

      fprintf(outp, "Enter length in z-direction (cm): ");
    ...
    {
      fprintf(outp, "Enter length of La (cm): ");

      if (fscanf(fp, "%lf", &La) == EOF)
        Error(0, "Missing La");

      fprintf(outp, "Enter length of Lb (cm): ");

      if (fscanf(fp, "%lf", &Lb) == EOF)
        Error(0, "Missing Lb");

      fprintf(outp, "Enter length of Lc (cm): ");

      if (fscanf(fp, "%lf", &Lc) == EOF)
        Error(0, "Missing Lc");

      fprintf(outp, "Enter Psi angle (degrees): ");

      if (fscanf(fp, "%lf", &ps) == EOF)
        Error(0, "Missing psi");

      fprintf(outp, "Enter Theta angle (degrees): ");

      if (fscanf(fp, "%lf", &th) == EOF)
        Error(0, "Missing theta");

      fprintf(outp, "Enter Phi angle (degrees): ");
  ...
  fprintf(outp, "Randomizing %ld particles for initial sampling...\n\n", 
	  Nmax);
  ...
  if (growth > 0.0)
      ...
      do
	{
	  ...
	  if (rpfracprev != rpfracmin)
	    {
	      fprintf(outp, "Smallest particle radius fraction: %1.4f\n",
		      rpfracmin);
  ...
  for (n = 0; n < Nmax; n++)
    {
      fprintf(outp, "Verifying distribution: %6ld / %ld \n", n + 1, Nmax);
  ...
  fprintf(outp, "%ld particles, packing fraction = %1.5f\n\n", Nmax, V1/V0);
