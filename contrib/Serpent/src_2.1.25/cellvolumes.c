...
void CellVolumes()
{
  long cell, loc0, loc1, loc2, surf, ptr, n, type;
  ...
  while (loc0 > VALID_PTR)
    {
      ...
      loc2 = (long)RDB[loc1 + IFC_PTR_TET_MSH];
      ...
      while (loc2 > VALID_PTR)
	{
	  ...
	  cell = (long)RDB[loc2 + IFC_TET_MSH_PTR_CELL];
	  CheckPointer(FUNCTION_NAME, "(cell/tet)", DATA_ARRAY, cell);
	  
	  /* Add volume to cell */

	  WDB[cell + CELL_VOLUME] += TetraVol(loc1, loc2);
