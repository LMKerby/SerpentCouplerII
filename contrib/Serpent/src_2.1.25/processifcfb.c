...
void ProcessIFCFB(long loc0, long update)
{
  ...
  if ((long)RDB[DATA_GEOM_DIM] == 2)
	...
	{
	  /* Modify this in all of the time bins */
      ...
      while (loc1 > VALID_PTR)
	{
	  ...
	  for (m = 0; m < (long)RDB[loc1 + IFC_FUEP_N_UNI]; m++)
	      ...
	      while (tbi > VALID_PTR)
		{
		  ...
		  /* TODO: Voisko tän tehdä jo readifcfb.c:ssä näitä lukiessa? */
	      ...
	      while (reg > VALID_PTR)
		{
		  ...
		  if ((mat = (long)RDB[cell + CELL_PTR_MAT]) > VALID_PTR)
		      ...
		      if ((mat0=(long)RDB[mat + MATERIAL_DIV_PTR_PARENT]) > VALID_PTR)
			{
			  ...
			  if (RDB[mat0 + MATERIAL_TMS_TMIN] <=
			       RDB[mat0 + MATERIAL_TMS_TMAX] )
			  ...
			  if ((RDB[mat0 + MATERIAL_TMS_TMIN] <
			       RDB[mat0 + MATERIAL_TMS_TMAX] ) ||
			      ((long)RDB[mat0 + MATERIAL_PTR_MIX] > VALID_PTR))
			    WDB[mat0 + MATERIAL_TMS_MODE] = (double)YES;
			  else if (RDB[mat + MATERIAL_TMS_TMIN] == RDB[mat + MATERIAL_TMS_TMAX])
			    WDB[mat0 + MATERIAL_DOPPLER_TEMP] = RDB[loc0 + IFC_MIN_TEMP];
