		  ...
		  if (((tgt = (long)RDB[loc0 + FY_PTR_TGT]) > VALID_PTR) &&
		      (tgt != (long)RDB[DATA_PTR_NUCLIDE_LOST]))
		    {
		      ...
		      if (!CompareStr(nuc + NUCLIDE_PTR_LIB_ID,
				      tgt + NUCLIDE_PTR_LIB_ID))
                        { 
			Warn(FUNCTION_NAME, "Mismatch in library ID 2"); /* LMK */
                        fprintf (err, "nuc = %s, tgt = %s", GetText(nuc + NUCLIDE_PTR_LIB_ID), GetText(tgt + NUCLIDE_PTR_LIB_ID)); /* LMK */
                        }
