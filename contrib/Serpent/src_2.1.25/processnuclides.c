...
{
  long mat, src, iso, ace, nuc, loc0, ptr, ZAI, n, TMS;
  ...
  while (mat > VALID_PTR)
    {
      ...
      if ((long)RDB[mat + MATERIAL_DIV_PTR_PARENT] < VALID_PTR)
	{
	  fprintf(out, "Adding nuclides in material %s...\n\n",
  ...
  if (((long)RDB[DATA_BURNUP_CALCULATION_MODE] == YES) &&
      ((long)RDB[DATA_BURN_DECAY_CALC] == NO))
    {
      ...
      fprintf(out, "Adding activation products...\n\n");
  ...
  if ((long)RDB[DATA_BURN_DECAY_CALC] == NO)
      ...
      while (loc0 > VALID_PTR)
	{
	  ...
	  if ((ptr = (long)RDB[DATA_PTR_AC_ZAI_LIST]) > VALID_PTR)
		...
		fprintf(out, "Adding actinide data...\n\n");
	  ...
	  if ((ptr = (long)RDB[DATA_PTR_FP_ZAI_LIST]) > VALID_PTR)
	    {
	      ...
	      if (PrevItem(loc0) < VALID_PTR)
		fprintf(out, "Adding fission product data...\n\n");
  ...
  if ((long)RDB[DATA_BURNUP_CALCULATION_MODE] == YES)
    {
      ...
      fprintf(out, "Adding remaining data and forming paths...\n");
  ...
  while (src > VALID_PTR)
      ...
      if ((long)RDB[src + SRC_PTR_XSDATA] > VALID_PTR)
	{
	  ...
	  if (n++ == 0)
	    fprintf(out, "Adding nuclides for sources...\n\n");
  ...
  if ((long)RDB[DATA_TMS_MODE] != TMS_MODE_NONE)
    {
      fprintf(out, "Setting majorant temperatures for TMS:\n\n");
      ...
      while (mat > VALID_PTR)
	{
	  ...
	  if ((RDB[mat + MATERIAL_TMS_MODE] != TMS_MODE_NONE) &&
		...
		Die(FUNCTION_NAME, "Zero TMS temperatures"); 
	      ...
	      if (Tmin < Tmax)
		fprintf(out, "Material %s between %1.1fK and %1.1fK\n", 
			GetText(mat + MATERIAL_PTR_NAME), Tmin, Tmax);
	      else
		fprintf(out, "Material %s at %1.1fK\n", 
  ...
  if (((ptr = (long)RDB[DATA_PTR_DBRC]) > VALID_PTR) && 
      ((long)RDB[DATA_USE_DBRC] == YES))
    {
      ...
      fprintf(out, "Adding 0K data for DBRC...\n\n");
  ...
  if ((ptr = RDB[DATA_PTR_T0]) > VALID_PTR)
    {
      fprintf(out, "Adding thermal scattering data...\n\n");
