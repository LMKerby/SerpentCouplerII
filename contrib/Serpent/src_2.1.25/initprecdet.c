...
void InitPrecDet()
{
  ...
  double val, relerr, lambda, *filelams, wcount[8], acount[2][8];
  double x, y, z, u, v, w, E, wgt, t, atot[2],t0;
  char tmpstr[MAX_STR];
  FILE *fp;
  ...
  if (RDB[DATA_PRECURSOR_TRANSPORT_MODE] == PREC_MODE_POINT)
	...
	Die(FUNCTION_NAME, "Could not open dyn source file %s for reading", tmpstr);
      ...
      /* Get 1.5*nsrc neutrons from file */
      /* TODO: more? less?               */
 
      maxi = (long)RDB[DATA_SRC_POP]*1.5;
	...
	fprintf(out, "%6f ", wcount[i]);
      ...
      fprintf(out,"Betas in mesh:\n");

      for (i = 0; i < 8; i++)
	fprintf(out, "%6f ", acount[0][i]/atot[0]);

      fprintf(out,"\n");

      fprintf(out,"Betas in list:\n");

      for (i = 0; i < 8; i++)
	fprintf(out, "%6f ", acount[1][i]/atot[1]);
