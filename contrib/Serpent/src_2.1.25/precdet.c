...
{
  long loc0, loc1, rea, nuc, ptr, rls, lst, n, np, i, ibin, tbin, gbin;
  long stp, erg, ng, new, prg, prev;
  double dnu, rr, f, P, P0, P1, Pemit, Ptal, val, adens, dt, lambda, nprec;
  double macroxs, Nemit, Wemit, Wtresh, prob, temit, Etal, Etresh, Dt;
  double x, y, z, t, u, v, w, E, mu, wgt2, norm, Wtal, impP;
      ...
      while ((loc1 = ListPtr(lst, n++)) > VALID_PTR)
	{
	  ...
	  if ((RDB[DATA_SIMULATION_MODE] != SIMULATION_MODE_CRIT) && 
	      (RDB[DATA_PRECURSOR_TRANSPORT_MODE] == PREC_MODE_POINT))
	    {
	      ...
	      /* Calculated first in decayprecdet.c */
	      /* And later in getbankedprecursors.c */
	      ...
	      if (prob > 1.0)
		Die(FUNCTION_NAME, "Too many precursors produced, should implement this one");
...
#ifdef DNPRINT
		  fprintf(out, "Emission from this would have been %E\n", Etal);
		  fprintf(out, "Emission from this will be %E\n", Etal/prob);
#endif
	  ...
	  if (Wemit < Wtresh)
	      ...
	      if (RandF(id) < prob)
		{
		  ...
		  */
		  ...
		  ScoreFission(mat, rea, tnu, dnu, lambda, ng, 0.0, E, wgt0, wgt2, 
			       idx0, idx1, id);      
		  */
