  ...
  for (nr = 0; nr < NTRP; nr++)
      ...
      if ((MFTYPE == 12) || (MFTYPE == 16))
	{
	  ...
	  if (rea > VALID_PTR)
	    ...
	    Die(FUNCTION_NAME, "Reaction %ld not found", MTMULT);
	  ...
	  if (NR < 2)
		...
		Warn(FUNCTION_NAME, "Log-log interpolation of xs");
	      ...
	      InterpolateData(E0, xs1, NES, &XSS[L0 + 2*NR + 3], 
			      &XSS[L0 + 2*NR + 3 + NE], NE, imode, NULL, NULL);
	    }
	  else
	    {
	      printf("\n-------------------------\n");
      ...
      if (LOCA != 0)
	{
		  ...
		  for (i = 0; i < 33; i++)
		    {
		      ...
		      mu = XSS[L1 + i];
		      CheckValue(FUNCTION_NAME, "mu", " (mu)", mu, -1.0, 1.0);
      ...
      if ((long)XSS[L0 - 1] != 0)
	Die(FUNCTION_NAME, "LNW = %ld", (long)XSS[L0 - 1]);
