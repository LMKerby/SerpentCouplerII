...
void StatTests()
{
  ...
  fprintf(out, "Performing statistical tests on output variables...\n");
...
{ 
  ...
  long i, j, Nbatch, Ni, batch_size, Ninactive, Nactive;
  double scoresum;
  ...
  is_batchsize_1 = 0;
  for (i = 0; i < sizeof(params_batchsize_1)/sizeof(params_batchsize_1[0]); i++)
  ...
  is_gcu = 0;
  if (!is_batchsize_1) {
    for (i = 0; i < sizeof(paramspre_gcu)/sizeof(paramspre_gcu[0]); i++)
...
static long ShapiroWilkTest(double *x, long N, double *Z_W_R, double *p_W_R)
{
  ...
  for (i = 0; i < N; i++) {
    ...
    if (!NormCDFInv((i + 1 - 0.375)/(N + 0.25), mtilde + i)) {
  ...
  if (N <= 5) {
    ...
    sqrtphi = sqrt((mtildess - 2.0*mtilde[N-1]*mtilde[N-1] - 2*mtilde[N-2]*mtilde[N-2]) /
...
static long DAgostinoPearsonTest(double *x, long N, double *K2, double *p_K2)
{
  ...
  if (SkewnessTest(x, N, &Z_b1sqrt, &p_tmp) && KurtosisTest(x, N, &Z_b2, &p_tmp)) {
    *K2 = Z_b1sqrt*Z_b1sqrt + Z_b2*Z_b2;

    /* K2 has approximately a chi-squared distribution with 2 degrees of freedom.
    The exact cumulative distribution function for this case is: */
...
static long SkewnessTest(double *x, long N, double *Z_b1sqrt, double *p_b1sqrt)
{
   ...
   * Ralph B. D'agostino, Albert Belanger, Ralph B. D'agostino, "A suggestion for
  ...
  /* Avoid division by zero (m_2 == 0, when all the elements of x have the same value) */
  ...
  Y = b1sqrt*sqrt((N + 1.0)*(N + 3.0)/(6.0*(N - 2.0)));
  tau = 3.0*(N*N + 27.0*N - 70.0)*(N + 1.0)*(N + 3.0) / ((N - 2.0)*(N + 5.0)*(N + 7.0)*(N + 9.0));
  omega2 = sqrt(2.0*(tau - 1.0)) - 1;
  
  /* When N is very large (N > 10^15), omega2 -> 1, and the test can't be performed */
  if (omega2 - 1.0 < ZERO)
    return 0;
...
static long KurtosisTest(double *x, long N, double *Z_b2, double *p_b2)
{
  ...
  Eb2 = 3.0*(N - 1.0)/(N + 1.0);
  varb2 = 24.0*N*(N - 2.0)*(N - 3.0)/((N + 1.0)*(N + 1.0)*(N + 3.0)*(N + 5.0));
  xi = (b2 - Eb2)/sqrt(varb2);
  beta1sqrt = 6.0*(N*N - 5.0*N + 2.0)/((N + 7.0)*(N + 9.0))*sqrt(6.0*(N + 3.0)*(N + 5.0) /
              (N*(N - 2.0)*(N - 3.0)));
  A = 6.0 + 8.0/beta1sqrt*(2.0/beta1sqrt + sqrt(1.0 + 4.0/(beta1sqrt*beta1sqrt)));
  ...
  if (1.0 + xi*sqrt(2.0/(A - 4.0)) <= ZERO) {
    ...
    *Z_b2 = sqrt(9.0*A/2.0)*((1.0 - 2.0/(9.0*A)) - pow((1.0 - 2.0/A)/(1.0 + xi*sqrt(2.0/(A - 4.0))),
           1.0/3.0));
...
/*****************************************************************************/
static long DriftInMeanTest(double *x, long N, double *Z_drift, double *p_drift, long m)
{
  ...
  for (i = 0; i < m-1; i++) {
    ...
    if ((var[i] == 0.0) || (var[i+1] == 0.0)) {
      ...
      Z_drift[i] = (mean[i+1] - mean[i])/sqrt(var[i]/Ni[i] + var[i+1]/Ni[i+1]);
      
      /* Z_drift is t-distributed with nu degrees of freedom */
      
      u = var[i+1]/var[i];
      tmp = 1.0/Ni[i] + u/Ni[i+1];
      nu = tmp*tmp / (1.0/(Ni[i]*Ni[i]*(Ni[i] - 1.0)) + 
                      u*u/(Ni[i+1]*Ni[i+1]*(Ni[i+1] - 1.0)));
...
/*****************************************************************************/
static long Betareg(double x, double a, double b, double tol, long maxit, double *betareg)
{
...
{
  /*  Evaluates a polynomial of degree 'degree' with coefficients 'coeff' at 'x'.
   * */
