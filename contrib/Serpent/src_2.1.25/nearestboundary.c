...
double NearestBoundary(long id)
{
  ...
  long nbhr, uni, ifc, ncol, tbi, ang, nr, i, cgns, loc1,  nt;
  double min, d, x, y, z, u, v, w, r2, y2, z2, params[MAX_SURFACE_PARAMS];
  double t, phi, phi2;
	...
	case UNIVERSE_TYPE_NEST:
	  {
		...
		/* Find the radial zone where the point is */

		ptr = (long)RDB[ang + IFC_FUEP_ANG_PTR_HOT_R2];
		CheckPointer(FUNCTION_NAME, "(ptr)", DATA_ARRAY, ptr);

		i = SearchArray(&RDB[ptr], r2, nr);
		...
		if (i > -1)
		  {
		    ...
		    params[0] = 0.0;
		    params[1] = 0.0;
		    params[2] = sqrt(RDB[ptr + i + 1]);
		    d = SurfaceDistance(-1, params, SURF_CYL, 3, 
					x, y, z, u, v, w, id);
		    CheckValue(FUNCTION_NAME, "d", "7", d, 0.0, INFTY);
		    ...
		    params[0] = 0.0;
		    params[1] = 0.0;
		    params[2] = sqrt(RDB[ptr + i]);
		    d = SurfaceDistance(-1, params, SURF_CYL, 3, 
					x, y, z, u, v, w, id);
		    CheckValue(FUNCTION_NAME, "d", "8", d, 0.0, INFTY);
		...
		/* if point was outside the outermost input point */

		/* Distance to outer radius */
			
		params[0] = 0.0;
		params[1] = 0.0;
		params[2] = sqrt(RDB[ptr + nr - 1]);
		d = SurfaceDistance(-1, params, SURF_CYL, 3, 
					x, y, z, u, v, w, id);
		CheckValue(FUNCTION_NAME, "d", "9", d, 0.0, INFTY);
