...
void ReadInput(char *inputfile)
{
  ...
  while ((i = NextWord(&input[i0], word)) > 0)
    {
      ...
      if (lines > 10000)
	{
	  /* Switch line number checking off for large input files */
	  
	  WDB[DATA_NO_LINE_NUMBER_CALC] = (double)YES;
	}
      else
	WDB[DATA_NO_LINE_NUMBER_CALC] = (double)NO;
      ...
      else if (!strcasecmp(word, "branch"))
	{
	  ...
	  while (j < np)
	      ...
	      else if (!strcmp(params[j], "tra"))
		{
		  ...
		  if (j == np)
		    Error(loc0, "Missing first transformation");
		  else
		    WDB[loc1 + DEP_BRA_TRANS_PTR_TRANS1] =  
		      (double)PutText(params[j++]);

		  /* Read second transformation */
		  
		  if (j == np)
		    Error(loc0, "Missing second transformation");
		  else
		    WDB[loc1 + DEP_BRA_TRANS_PTR_TRANS2] =  
		      (double)PutText(params[j++]);
	      ...
	      else if (n == 1)
		{
		  ...
		  if (j == np)
		    Error(loc0, "Missing S(a,b) library");
		  else
		    WDB[loc2 + DEP_BRA_STP_SAB_PTR_LIB] =  
		      (double)PutText(params[j++]);

		  /* Read ZA */
		  
		  if (j == np)
		    Error(loc0, "Missing S(a,b) ZA");
		  else
		    WDB[loc2 + DEP_BRA_STP_SAB_ZA] =  
      ...
      else if (!strcasecmp(word, "tfb"))
	{
	  ...
	  params = GetParams(word, input, &np, &i0, 6, 306, fname);
	  ...
	  /* Read mode number */
          ...
          WDB[loc0 + TFB_MODE] = 
	    (double)TestParam(pname, fname, line, params[j++], PTYPE_INT, 0, 2);
	  ...
	  while (j < np)
	    {
	      ...
	      WDB[ptr + TFB_REG_TMS_TMIN] = INFTY;
	      WDB[ptr + TFB_REG_TMS_TMAX] = -INFTY;
		...
		WDB[ptr + TFB_REG_PTR_MAT] = PutText(params[j++]);
	      ...
	      if (j == np)
		Error(loc0, "Missing minimum temperature");
	      else
		WDB[ptr + TFB_REG_TMS_TMIN] = 
		  TestParam(pname, fname, line, params[j++], PTYPE_REAL, 1.0, 
			    1E+6);

	      Warn(FUNCTION_NAME, "toi minimi ei saa olla nolla");

	      /* Read maximum temperature */
	      
	      if (j == np)
		Error(loc0, "Missing maximum temperature");
	      else
		WDB[ptr + TFB_REG_TMS_TMAX] = 
		  TestParam(pname, fname, line, params[j++], PTYPE_REAL, 
			    RDB[ptr + TFB_REG_TMS_TMIN], 1E+6);
      ...
      else if (!strcasecmp(word, "fin"))
	{
	  ...
	  if (j == np)
	    ...
	    WDB[loc0 + FINIX_RODTYPE] = 
	      (double)TestParam(pname,fname,line,params[j++], 
				PTYPE_INT, -1, 
				1000);

	  if(RDB[loc0 + FINIX_RODTYPE] < 0)
	    {
	      /* Read rod definition file */

	      if (j == np)
		Error(loc0, "Missing boundary condition type");
	      else
		WDB[loc0 + FINIX_PTR_FNAME] = PutText(params[j++]);
	    }
	  ...
	  if (j == np)
	    ...
	    WDB[loc0 + FINIX_BCTYPE] = 
	      (double)TestParam(pname,fname,line,params[j++], 
				PTYPE_INT, FINIX_BCTYPE_CLADT, 
				FINIX_BCTYPE_COOLT);	  

	  /* Read number of axial FINIX nodes*/

	  if (j == np)
	    Error(loc0, "Missing number of axial nodes");
	  else
	    WDB[loc0 + FINIX_NZ] = 
	      (double)TestParam(pname,fname,line,params[j++], 
				PTYPE_INT, 1, 100000);
	  ...
	  if (j == np)
	    Error(loc0, "Missing number of fuel radial nodes");
	  else
	    WDB[loc0 + FINIX_NR_FUEL] = 
	      (double)TestParam(pname,fname,line,params[j++], 
				PTYPE_INT, 1, 100000);

	  if (j == np)
	    Error(loc0, "Missing number of cladding radial nodes");
	  else
	    WDB[loc0 + FINIX_NR_CLAD] = 
	      (double)TestParam(pname,fname,line,params[j++], 
				PTYPE_INT, 1, 10000);

	  if (j == np)
	    Error(loc0, "Missing number of axial power nodes");
	  else
	    WDB[loc0 + FINIX_NZ_POW] = 
	      (double)TestParam(pname,fname,line,params[j++], 
				PTYPE_INT, 1, 10000);

	  if (j == np)
	    Error(loc0, "Missing axial lower boundary");
	  else
	    WDB[loc0 + FINIX_ZMIN] = 
	      TestParam(pname,fname,line,params[j++], 
			PTYPE_REAL, -1000000.0, 1000000.0);

	  if (j == np)
	    Error(loc0, "Missing axial upper boundary");
	  else
	    WDB[loc0 + FINIX_ZMAX] = 
	      TestParam(pname,fname,line,params[j++], 
			PTYPE_REAL, RDB[loc0 + FINIX_ZMIN], 1000000.0);

	  if (j == np)
	    Error(loc0, "Missing number of radial power nodes");
	  else
	    WDB[loc0 + FINIX_NR_POW] = 
	      (double)TestParam(pname,fname,line,params[j++], 
				PTYPE_INT, 1, 10000);

	  if (j == np)
	    Error(loc0, "Missing fuel inradius");
	  else
	    WDB[loc0 + FINIX_RMIN] = TestParam(pname,fname,line,params[j++], 
					       PTYPE_REAL, 0.0, 100.0);

	  if (j == np)
	    Error(loc0, "Missing fuel outradius");
	  else
	    WDB[loc0 + FINIX_RMAX] = 
	      TestParam(pname,fname,line,params[j++], 
			PTYPE_REAL, RDB[loc0 + FINIX_RMIN], 100.0);

	  if (j == np)
	    Error(loc0, "Missing clad inradius");
	  else
	    WDB[loc0 + FINIX_CMIN] =
	      TestParam(pname,fname,line,params[j++], 
			PTYPE_REAL, RDB[loc0 + FINIX_RMAX], 100.0);

	  if (j == np)
	    Error(loc0, "Missing clad outradius");
	  else
	    WDB[loc0 + FINIX_CMAX] = 
	      TestParam(pname,fname,line,params[j++], 
			PTYPE_REAL, RDB[loc0 + FINIX_CMIN], 100.0);

	  
	  /* Create the regions and read the boundary conditions */
	  
	  for (n = 0; n < (long)RDB[loc0 + FINIX_NZ]; n++)
	    {
	      loc1 = NewItem(loc0 + FINIX_PTR_AX, FINIX_AX_BLOCK_SIZE);
	      
	      WDB[loc1 + FINIX_AX_IDX] = (double)n;
	      WDB[loc1 + FINIX_AX_ZMIN] = 
		n*(RDB[loc0 + FINIX_ZMAX] - 
		   RDB[loc0 + FINIX_ZMIN])/RDB[loc0 + FINIX_NZ];
	      
	      WDB[loc1 + FINIX_AX_ZMAX] = 
		(n+1.0)*(RDB[loc0 + FINIX_ZMAX]-
			 RDB[loc0 + FINIX_ZMIN])/RDB[loc0 + FINIX_NZ];

	      /* Read the boundary condition */

	      if((long)RDB[loc0 + FINIX_BCTYPE] == FINIX_BCTYPE_CLADT)
		{
		  if (j == np)
		    Error(loc0, "Missing a BC");
		  else
		    WDB[loc1 + FINIX_AX_CLADT] = 
		      TestParam(pname,fname,line,params[j++], 
				PTYPE_REAL, 0.0, 4000.0);
		}
	      else if((long)RDB[loc0 + FINIX_BCTYPE] == FINIX_BCTYPE_HFLUX)
		{
		  if (j == np)
		    Error(loc0, "Missing a BC");
		  else
		    WDB[loc1 + FINIX_AX_HFLUX] = 
		      TestParam(pname,fname,line,params[j++], 
				PTYPE_REAL, 0.0, 4000.0);
	      }
	      else if((long)RDB[loc0 + FINIX_BCTYPE] == FINIX_BCTYPE_HTCOE)
		{
		  if (j == np)
		    Error(loc0, "Missing a BC");
		  else
		    WDB[loc1 + FINIX_AX_HTCOE] = 
		      TestParam(pname,fname,line,params[j++], 
				PTYPE_REAL, 0.0, 400000.0);
		  
		  if (j == np)
		    Error(loc0, "Missing a BC");
		  else
		    WDB[loc1 + FINIX_AX_COOLT] = 
		      TestParam(pname,fname,line,params[j++], 
				PTYPE_REAL, 0.0, 4000.0);
	      }
	      else if((long)RDB[loc0 + FINIX_BCTYPE] == FINIX_BCTYPE_COOLT)
		{
		  if (j == np)
		    Error(loc0, "Missing a BC");
		  else
		  ...
		  if ((long)RDB[loc1 + DET_RBIN_MT] == MT_USER_DEFINED)
		    {
		      ...
		      if (j == np)
			Error(loc0, "Missing interpolation after \"%s\"", str);
		      else
			WDB[loc1 + DET_RBIN_USDEF_INTT] = 
			  TestParam(pname, fname, line, params[j++], PTYPE_INT,
				    1, 5);

		      /* Read number of energy-value pairs */

		      if (j == np)
			Error(loc0, "Missing number of values after \"%s\"", 
			      str);
		      else
			ne = TestParam(pname, fname, line, params[j++], 
				       PTYPE_INT, 2, 1000000);

		      /* Put value */

		      WDB[loc1 + DET_RBIN_USDEF_NP] = (double)ne;

		      /* Allocate memory for data */

		      loc2 = ReallocMem(DATA_ARRAY, ne);
		      WDB[loc1 + DET_RBIN_USDEF_PTR_E] = (double)loc2;
		      
		      loc3 = ReallocMem(DATA_ARRAY, ne);
		      WDB[loc1 + DET_RBIN_USDEF_PTR_F] = (double)loc3;
			...
			{
			  if (j > np - 2)
			    Error(loc0,"Invalid number of values after \"%s\"", 
				  str);
      ...
      else if (!strcasecmp(word, "set"))
	{
	  ...
	  if (!strcasecmp(params[j], "dfsol"))
	  ...
	  else if (!strcasecmp(params[j], "cax"))
	    {
	      /***** Write card image file ***********************************/
		...
		WDB[DATA_CAX_OUTPUT] = 
	      ...
	      /* Set option to collect simulator data */

	      if ((long)RDB[DATA_CAX_OUTPUT] == YES)
		WDB[DATA_SIMULATOR_DATA] = (double)YES;
		...
		WDB[DATA_N_POP_EIG] = 
		  TestParam(pname, fname, line, params[k++], PTYPE_INT, 
              ...
              /* Set maximum population to simulate in coupled calculation */

              WDB[DATA_SOL_REL_MAXPOP] = RDB[DATA_CRIT_POP]*RDB[DATA_CRIT_CYCLES];
	    ...
	    {
	      /****** Parameters not allowed in Serpent 2 ********************/
	      ...
	      Error(-1, params[j], fname, line,
		    "Method not supported in Serpent 2");
  ...
  /* Switch line number checking back on */
