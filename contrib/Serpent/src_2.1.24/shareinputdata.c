...
void ShareInputData()
{
  ...
  fprintf(out, "Sharing input data to MPI tasks...\n");
  ...
  if (MPI_Bcast(&sz, 1, MPI_LONG, 0, MPI_COMM_WORLD) != MPI_SUCCESS)
  ...
  if (MPI_Bcast(ASCII, sz, MPI_CHAR, 0, MPI_COMM_WORLD) != MPI_SUCCESS)
    Die(FUNCTION_NAME, "MPI Error");
  ...
  if (MPI_Bcast(&parent_seed, 1, MPI_LONG, 0, MPI_COMM_WORLD) != MPI_SUCCESS)
    Die(FUNCTION_NAME, "MPI Error");
  ...
  /* Estimated total number of particles for adjusting RNG */

  n = (long)RDB[DATA_BURN_TOT_STEPS] + 1;
  
  if ((long)RDB[DATA_BURN_CORR_TYPE] != CORR_TYPE_NONE)
    n = n*2;

  if ((long)RDB[DATA_CRIT_CYCLES] > 0)
    n = n*((long)RDB[DATA_CRIT_CYCLES] + (long)RDB[DATA_CRIT_SKIP]);
  else
    n = n*((long)RDB[DATA_SRC_BATCHES]);

  if ((long)RDB[DATA_CRIT_POP] > 0.0)
    n = n*((long)(1.5*RDB[DATA_CRIT_POP]));
  else
    n = n*((long)(1.5*RDB[DATA_SRC_POP]));

  WDB[DATA_MPI_TOT_PARTICLES] = (double)n;
