...
void ComptonScattering(long rea, long part, double *E0, double x, double y,
  ...
  double mu, E, Ek0, Ee0, Ee, Sxt, xtmax, Sxtmax, xt, pzmaxd, pzmax,
      ...
      we, uq, vq, wq, u0, v0, w0, Edeploc, q, pe0c, pec, cbeta, cdelta, calpha;
  ...
  *E0 = E;
  ...
  if ((long)RDB[DATA_PHOTON_USE_DOPPLER] && (long)RDB[DATA_PHOTON_COMP_EANG]) {
     ...
     * transfer vector. This equals to the free-electron scattering angle when
  ...
  TTB(rea, part, Ee, x, y, z, ue, ve, we, wgt, t, 0, id);
  

  /***** Energy deposition and atomic relaxation *****************************/
  ...
  if (Uss < RDB[DATA_PHOTON_EMIN]) {
    ...
    Nssar = (long)RDB[ptd + PHOTON_DIST_N_AR_NSS];

    /* Pointer to shell eletron number CDF of the relaxation model */
    ptrelncdfar = (long)RDB[ptd + PHOTON_DIST_PTR_AR_ELNCDF];
    ...
    if (ss == -1) {
      ...
      /* Atomic relaxation */
      Edeploc = AtomicRelaxation(rea, part, ss, x, y, z, wgt, t, id);
