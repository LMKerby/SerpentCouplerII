...
void CalculateRelPopSize()
{
  long loc0, nb;
  long bleft;
  double alpha, popleft;

#ifndef nobatch
  long loc1, ptr, ptr1, nz, nr, i, j, k, l, na, nt, type, idx, ptr2, iter;
  double ncur, ntot, pn, prod, upper, lower, vn1, x0, fx, dfx, sn1;
  ...
  /*
  fprintf(out, "iter = %ld, fx = %E, pn = %E (%E/%E), vn1 = %E, Sn = %ld, Sn+1 = %ld, sn+1 = %ld, s1 = %ld, alpha = %E\n", iter, fx, pn,upper,lower, vn1, (long)ntot, (long)x0, (long)(x0-ntot), (long)(alpha*alpha*ntot), alpha);
  fprintf(out, "%.6E %ld %.6E %% Res\n",pn, (long)ntot, alpha);
  */
  ...
  if ((long)RDB[DATA_SIMULATION_MODE] == SIMULATION_MODE_CRIT)
    {
      ...
      popleft = RDB[DATA_SOL_REL_MAXPOP] - RDB[DATA_SOL_REL_NTOT];
...
/* Get next sample size */
  ...
  if ((long)RDB[DATA_SIMULATION_MODE] == SIMULATION_MODE_CRIT)
    {  
      ...
      WDB[DATA_SOL_REL_NCUR] = nb*RDB[DATA_CRIT_POP];
  ...
  /*
  fprintf(out, "New batch interval %ld\n", nb);
  */
