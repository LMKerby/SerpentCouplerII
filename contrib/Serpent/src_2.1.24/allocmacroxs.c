...
void AllocMacroXS()
{
  long mat, mat0, sz, mode, loc0, loc1, n, lst, rea, ptr, erg, ne, nr, iso;
  long nuc;
      ...
      for (n = 0; n < 12; n++)
	{
	  ...
	  if ((loc0 = (long)RDB[mat0 + mode]) < VALID_PTR)
	  ...
	  else
	    {
	      ...
	      WDB[loc1 + RLS_PTR_REA0] = WDB[loc0 + RLS_PTR_REA0];
	    }
	}
  ...
  while (mat > VALID_PTR)
      ...
      for (n = 0; n < 10; n++)
	{
	  ...
	  AllocValuePair(rea + REACTION_PTR_PREV_XS0);
	}

      /***********************************************************************/
      
      /***** Allocate memory for unresolved resonance data *******************/
      ...
      for (n = 0; n < 6; n++)
	{
	  ...
	  if (n == 0)
	    {
	      ...
	      lst = (long)RDB[mat + MATERIAL_PTR_TOT_URES_LIST];
	    }
	  else if (n == 1)
	    {
	      rea = (long)RDB[mat + MATERIAL_PTR_ABSXS];
	      lst = (long)RDB[mat + MATERIAL_PTR_ABS_URES_LIST];
	    }	  
	  else if (n == 2)
	    {
	      rea = (long)RDB[mat + MATERIAL_PTR_ELAXS];
	      lst = (long)RDB[mat + MATERIAL_PTR_ELA_URES_LIST];
	    }	  
	  else if (n == 3)
	    {
	      rea = (long)RDB[mat + MATERIAL_PTR_FISSXS];
	      lst = (long)RDB[mat + MATERIAL_PTR_FISS_URES_LIST];
	    }
	  else if (n == 4)
	    {
	  ...
	  /* Check if ures block is already defined */
	  
	  if ((long)RDB[rea + REACTION_PTR_URES] > VALID_PTR)
	    Die (FUNCTION_NAME, "URES data already defined");

	  /* Allocate memory for data block */

	  ptr = NewItem(rea + REACTION_PTR_URES, URES_BLOCK_SIZE);
	  
	  /* Allocate memory for previous factor */
	  
	  AllocValuePair(ptr + URES_PTR_PREV_FACT);
	}
  ...
  /***** Direct reaction sampling lists **************************************/

  if ((long)RDB[DATA_OPTI_DIRECT_SAMPLE_LIST] == YES)
    {
      /* Loop over materials */
      ...
      while (mat > VALID_PTR)
	{
	  ...
	  while (iso > VALID_PTR)
	      ...
	      while (lst > VALID_PTR)
		{
		  ...
		  rea = (long)RDB[lst + RLS_DATA_PTR_REA];
		  CheckPointer(FUNCTION_NAME, "(rea)", DATA_ARRAY, rea);

		  /* Add to list */

		  loc0 = NewItem(mat + MATERIAL_PTR_DIRECT_REA_LIST, 
				SAMPLE_LIST_BLOCK_SIZE);

		  /* Put pointer to composition */

		  WDB[loc0 + SAMPLE_LIST_PTR_COMP] = (double)iso;

		  /* Put pointer to reaction */

		  WDB[loc0 + SAMPLE_LIST_PTR_REA] = (double)rea;

		  /* Allocate memory for counter */

		  ptr = AllocPrivateData(1, PRIVA_ARRAY);
		  WDB[loc0 + SAMPLE_LIST_PTR_COUNT] = (double)ptr;
      ...
      /* Link divided material lists to parents */
      ...
      while (mat > VALID_PTR)
	{
	  ...
	  WDB[mat0 + MATERIAL_PTR_DIRECT_REA_LIST] = 
	    WDB[mat + MATERIAL_PTR_DIRECT_REA_LIST];
