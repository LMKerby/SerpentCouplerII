...
void ProcessRelaxation(long loc0, long nuc) {
  ...
  long i, j, elementfound, Nss, subi, ntr, eln, maxdesignator, ptrd2i, ptrntr,
      ...
      ptrsubk2, ptretr2, ptrftr2, ptrelncdf;
  ...
  while (fgets(linebuf, Nbuf, fp)){
    ...
    if ((sscanf(linebuf, "Element %s\n", tmpstr) == 1) &&
      ...
      WDB[loc0 + PHOTON_DIST_N_AR_NSS] = Nss;
      ...
      WDB[loc0 + PHOTON_DIST_PTR_AR_D2IMAP] = ptrd2i;
      ...
      if (Nss > 0) {
        ...
        for (i = 0; i < Nss; i++) {
          ...
          if (fgets(linebuf, Nbuf, fp)) {
            ...
            WDB[ptreln++] = eln;
            WDB[ptrelncdf+i+1] = WDB[ptrelncdf+i] + eln;
          }
          else
            Die(FUNCTION_NAME, "Can't read relaxation data");
