...
void SampleENDFLaw(long rea, long erg, double Ein, double *Eout, double *mu,
		   long id)
{
  long law, l0, l1, l2, ne, nb, nr, type, i, j, k, l, m, n;
  long l4, nuc, np, mt, K1, K2, ptr;
  ...
  switch(law)
	...
	Die(FUNCTION_NAME, "Unsupported ENDF law %ld (%s)", 
	    GetText(nuc + NUCLIDE_PTR_NAME), law);
	...
	*Eout = d0 + (rnd1*(nb - 1.0) - k)*(d1 - d0);
	...
	CheckValue(FUNCTION_NAME, "d0 (law 3)", "", d0, -0.2, 35.0);
	...
	if ((i < ne - 1) && ((long)RDB[erg + ERG_INTERP] != 1))
	  {
	    ...
	    EE1 = RDB[l1] + r*(RDB[l2] - RDB[l1]);
	    EEk = RDB[l1 + K1 - 1] + r*(RDB[l2 + K2 - 1] - 
					 RDB[l1 + K1 - 1]);
	...
	for (j = 0; j < 100; j++)
	  {
	    ...
	    /*
	    CheckValue(FUNCTION_NAME, "p1 (law 4/44/61)", "", p1, -1E-3,1E+12);
	    CheckValue(FUNCTION_NAME, "p2 (law 4/44/61)", "", p2, -1E-3,1E+12);
	    */
	...
	if (law == 44)
	  {
	    ...
	    CheckValue(FUNCTION_NAME, "R1 (law 4/44/61)", "", R1, 0.0, 1.0);
	    CheckValue(FUNCTION_NAME, "R2 (law 4/44/61)", "", R2, 0.0, 1.0);
	    CheckValue(FUNCTION_NAME, "A1 (law 4/44/61)", "", A1, 0.0, 5.0);
	    CheckValue(FUNCTION_NAME, "A2 (law 4/44/61)", "", A2, 0.0, 5.0);
  ...
  /* Check that sampled energy is reasonable (limits are checked later) */

  if (Eout != NULL)
    if (!((*Eout > 1E-15) && (*Eout < 1000.0)))
