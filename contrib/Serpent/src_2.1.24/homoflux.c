...
void HomoFlux(long gcu)
{
  ...
  switch (type)
	...
	if (nc > 0)
	  {
	    surfs_n[4][0] = 0.5*SQRT2; 
	    surfs_n[4][1] = 0.5*SQRT2;
	    surfs_n[4][2] = 0.0; 
	    
	    surfs_n[5][0] = -0.5*SQRT2;
	    surfs_n[5][1] = 0.5*SQRT2;
	    surfs_n[5][2] = 0.0; 
	    
	    surfs_n[6][0] = -0.5*SQRT2;
	    surfs_n[6][1] = -0.5*SQRT2;
	    surfs_n[6][2] = 0.0; 
	    
	    surfs_n[7][0] = 0.5*SQRT2;
	    surfs_n[7][1] = -0.5*SQRT2;
	    surfs_n[7][2] = 0.0; 
	  }
	...
	/* Set origin for surface integration (W-S-E-N) */
	
	surfs_r[0][0] = 0.5*ds[1]; 
	surfs_r[0][1] = -0.5*ds[2]; 
	surfs_r[0][2] = 0.0;
	
	surfs_r[1][0] = 0.5*ds[1]; 
	surfs_r[1][1] = 0.5*ds[3];
	surfs_r[1][2] = 0.0;
	
	surfs_r[2][0] = -0.5*ds[0]; 
	surfs_r[2][1] = 0.5*ds[3];
	surfs_r[2][2] = 0.0;
	
	surfs_r[3][0] = -0.5*ds[0]; 
	surfs_r[3][1] = -0.5*ds[2];
	surfs_r[3][2] = 0.0;
  ...
  /***** Calculate coefficients for base functions (?) ***********************/
  ...
  for (j = 0; j < nG; j++)
	...
	CheckValue(FUNCTION_NAME, "scatt", "", scatt[i], ZERO, INFTY);
  ...
  */
  ...
  /* boundary condition system: b = -inv(D)*J */
  ...
  for (j = 0; j < nc; j++)
    for (i = 0; i < nG; i++)
      b[k++].re = -1.0/D[i]*Jc[n++]/dc[j];

  /* Solve coefficients c from boundary conditions */

  /* HUOM! tuonne dfsolveriin pitää myös välittää noi vektorit */

  dfSolver(ifCorn, nG, ns + nc, ds[0], dc[0], surfs_n, surfs_r, T, U, b, c); 
  ...
  /* Tässä myös noi pinnat erikseen */

  t = MakeArray(0.0, ds[0], Nr, 1);
  ...
  t = MakeArray(0.0, 0.5*dc[0], Nr, 1);

  /* First loop over corners */
  ...
  for (i = 0; i < nc; i++)
      ...
      for (k = 0; k < Nr; k++)
	{
	  ...
	  /* Kulmat ja pinnat erikseen */

	  r[0] = surfs_r[i][0] + (ds[0] - 0.5*dc[0] + t[k])*tn[0]; 
	  r[1] = surfs_r[i][1] + (ds[0] - 0.5*dc[0] + t[k])*tn[1]; 
	  r[2] = 0.0; 
	  /*
	  printf("ca%ld(%ld,:) = [%E %E];\n", i + 1, k + 1, r[0], r[1]);
	  */
      ...
      for (n = 0; n < nG; n++)
	{
	  ...
	  cflx[i*nG + n] = z.re/dc[i];
	} 
    }

  /* Second loop over corners */
  ...
  for (i = 0; i < nc; i++)
      ...
      for (k = 0; k < Nr; k++)
	{
	  ...
	  r[0] = surfs_r[j][0] + t[k]*tn[0]; 
	  r[1] = surfs_r[j][1] + t[k]*tn[1]; 
	  r[2] = 0.0; 
	  /*
	  printf("cb%ld(%ld,:) = [%E %E];\n", i + 1, k + 1, r[0], r[1]);
	  */
      ...
      for (n = 0; n < nG; n++)
	{
	  ...
	  cflx[i*nG + n] = cflx[i*nG + n] + z.re/dc[i];
	} 
    }
  ...
  */
  ...
  if (nneg > 0)
    Note(0, "Negative values in homogeneous flux solution");
...
/***** Print flux for plottin ************************************************/

void PrintFlx(long gcu, long surf, long nG, long nj, double **surfs_n, 
	      complex **T, complex **U, complex *c)
{
  long uni, type, ptr, nx, ny, n, m, i;
  double xmin, xmax, ymin, ymax, r[3], *f;
  FILE *fp;
  char tmpstr[MAX_STR];
  ...
  fprintf(fp, "x = [\n");
  for (n = 0; n < nx; n++)
    fprintf(fp, "%E\n", ((double)n)/((double)nx - 1.0)*(xmax - xmin) + xmin);
  fprintf(fp, "];\n");

  fprintf(fp, "y = [\n");
  for (m = 0; m < ny; m++)
    fprintf(fp, "%E\n", ((double)m)/((double)ny - 1.0)*(ymax - ymin) + ymin);
  fprintf(fp, "];\n");
  ...
  for (n = 0; n < nx; n++)
	...
	if (TestSurface(surf, r[0], r[1], r[2], NO, 0) == NO)
	  continue;
