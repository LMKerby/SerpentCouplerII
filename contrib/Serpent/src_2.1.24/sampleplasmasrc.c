		     ...
		     long id)
{
  ...
  long i2, i3, i4, i5, co;
  double rnd1, rnd2, rnd3, rnd4, rnd5, rho, X0, Y0, phi, Rp, Zp, R, Z;
  ...
  *z = 0.0;
  ...
  switch (co)
	...
	/* JLe 15/06/24: Noihin tarkistuksiin jotkut järkevät arvot */
    ...
    case 2:
      {
	N_rho = (long)RDB[loc1 + SRC_PLASMA_REA_N_RHO];
	CheckValue(FUNCTION_NAME, "N_rho", "", N_rho, 1, 1000);

        N_rea_xyb = (long)RDB[loc1 + SRC_PLASMA_REA_XYB];
	CheckValue(FUNCTION_NAME, "N_rea_xyb", "", N_rea_xyb, 1, 1000);
		
	ptr_rho = (long)RDB[loc2 + SRC_PLASMA_REA_PTR_R];
        CheckPointer(FUNCTION_NAME, "(ptr_r)", DATA_ARRAY, ptr_rho);
		
	ptr_p_rho = (long)RDB[loc2 + SRC_PLASMA_REA_PTR_P_RHO];
        CheckPointer(FUNCTION_NAME, "(ptr_r)", DATA_ARRAY, ptr_p_rho);
  ...
  if (N_e == 1)
    *E = RDB[ptr_e + 1];
    ...
    {
      i5 = SearchArray(&RDB[ptr_p_e], rnd5, N_e + 1);
