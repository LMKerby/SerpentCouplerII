  ...
  */
  ...
  if ((long)RDB[DATA_OPTI_MPI_REPRODUCIBILITY] == NO)
      ...
      while(mat > VALID_PTR)
	{
	  ...
	  /* Sort direct pointer list */

	  if ((lst = (long)RDB[mat + MATERIAL_PTR_DIRECT_REA_LIST]) 
	      > VALID_PTR)
	    SortList(lst, SAMPLE_LIST_PTR_COUNT, SORT_MODE_DESCEND_PRIVA);
