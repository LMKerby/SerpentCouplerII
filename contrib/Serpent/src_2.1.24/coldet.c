...
#define FUNCTION_NAME "ColdDet:"

/*****************************************************************************/

void ColDet(long part, long mat0, double flx0, double dE0, double x0, 
	    double y0, double z0, double u0, double v0, double w0, double E0, 
	    double t0, double wgt0, double g0, long id)
{
  long det0, loc0, idx0, det1, rbin0, ptr, type;
  double f0, val0, u, v, w;

#ifdef OLD_HIST
  
  long trk, mat1, hst0, hst, rbin1, idx1, loc1;
  ...
  while (det0 > VALID_PTR)
	...
	{     
	  ...
	  if ((idx0 = DetBin(det0, mat0, x0, y0, z0, E0, t0, rbin0, id)) < 0)
	    break;
	  ...
	  else if ((long)RDB[loc0 + DET_RBIN_MT] == MT_SOURCE_RATE)
	    {
	      /* Analog estimator of source rate (NOTE: only */
	      /* samplesrcpoint.c passes g0 = -1). */
	  ...
	  if ((det1 = (long)RDB[det0 + DET_PTR_ADJOINT]) < VALID_PTR)
	    {
	      ...
	      AddBuf1D(val0, wgt0, ptr, id, idx0);
		  ...
		  while (loc1 > VALID_PTR)
		    {
		      ...
		      if ((idx1 = DetBin(det1, mat1, x1, y1, z1, E1, t1, rbin1, 
					 id)) < 0)
			break;
