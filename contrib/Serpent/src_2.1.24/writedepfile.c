...
void WriteDepFile()
{
  ...
  if (((long)RDB[DATA_BURN_STEP] == 0) && 
      ...
      while (nuc > VALID_PTR)
	{
	  ...
	  val = RDB[nuc + NUCLIDE_SPEC_PHOTON_I]*RDB[nuc + NUCLIDE_LAMBDA];
	  fwrite(&val, sizeof(double), 1, fp);
	  ...
	  if (adens > ZERO)
	    {
	      ...
	      WDB[nuc + NUCLIDE_TOP_MASS] = WDB[nuc + NUCLIDE_TOP_MASS] 
		+ adens*RDB[nuc + NUCLIDE_AW]/N_AVOGADRO;

	      WDB[nuc + NUCLIDE_TOP_ACTIVITY] = WDB[nuc + NUCLIDE_TOP_ACTIVITY]
		+ adens*RDB[nuc + NUCLIDE_LAMBDA]/BARN;

	      WDB[nuc + NUCLIDE_TOP_SF] = WDB[nuc + NUCLIDE_TOP_SF]
		+ adens*RDB[nuc + NUCLIDE_SFBR]*RDB[nuc + NUCLIDE_LAMBDA]/BARN;

	      WDB[nuc + NUCLIDE_TOP_GSRC] = WDB[nuc + NUCLIDE_TOP_GSRC]
		+ adens*RDB[nuc + NUCLIDE_SPEC_PHOTON_I]*
		...
		RDB[nuc + NUCLIDE_LAMBDA]*MEV/BARN;
	      ...
	      WDB[nuc + NUCLIDE_TOP_ING_TOX] = WDB[nuc + NUCLIDE_TOP_ING_TOX] 
