...
double SampleMu(long rea, double E, long id)
{
  double mu, r, rnd, d1, d2, p1, p2, c1, c2, a, b, Emin, Emax;
  long ang, type, i, j, k, ne, np, nb, erg, l0;

  /* Check reaction pointer */

  CheckPointer(FUNCTION_NAME, "(rea)", DATA_ARRAY, rea);

  /* Use isotropic distribution for hydrogen to speed up calculation */

  if (RDB[rea + REACTION_AWR] < 1.0)
    if (E < 1E-3)
      {
	/* Sample */
  ...
  /* Get pointer to angular distribution data */
  ...
  CheckValue(FUNCTION_NAME, "ne", "", ne, 2, 25000);
  ...
  if ((type = (long)RDB[ang + ANG_TYPE]) == ANG_TYPE_EQUIBIN)
	...
	{      
	  ...
	  CheckValue(FUNCTION_NAME, "d1", "", d1, -1.0, 1.0);
	  CheckValue(FUNCTION_NAME, "d2", "", d2, -1.0, 1.0);
	  CheckValue(FUNCTION_NAME, "p1", "", p1, 0.0, 350.0);
	  CheckValue(FUNCTION_NAME, "p2", "", p2, 0.0, 350.0);
	  ...
	  if ((long)RDB[ang + ANG_INTT] == 1)
	  ...
	  else if ((long)RDB[ang + ANG_INTT] == 2)
	    {
	      ...
	      CheckValue(FUNCTION_NAME, "b", "", b, -1E-6, 1E+5);
  ...
  if ((mu == -1.0) || (mu == 0.0) || (mu == 1.0))
    Warn (FUNCTION_NAME, "Dubious value mu = %E sampled (mt = %ld, E = %E)",
