...
void Photoelectric(long rea, long part, double E, double x, double y,
  ...
  long ptd, ptr, Nssd, i, j, ss, ssmin, lo, Nxstotd, Nxsmin, lomin;
  double lE, Ee, beta, r1, ue, ve, we, totpexs, mue, if0, rtot, xsi;
  ...
  if (Nssd == 0 || E <= Ud[Nssd-1]) {
    ...
    if (ss == -1) {
      ...
      /* TODO: Store deposited energy? (the return value of AtomicRelaxation) */
      AtomicRelaxation(rea, part, ss, x, y, z, wgt, t, id);
    }
  ...
  /*mue = (1.0 + beta - 2.0*r1)/(1.0 + beta - 2.0*beta*r1);*/
  ...
  /* Use TTB-approximation for the photoelectron */
