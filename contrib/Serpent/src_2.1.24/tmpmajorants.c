...
void TmpMajorants()
{
  ...
  double *xs1, *maj, *min, *max, T0, T1, awr, f, maj0, *tempxs, elaxs;
  double qparam, sab_Emax;
#ifdef TRADMAJO
  double ar;
#else 
  long iter; 
  ...
  while (nuc > VALID_PTR)
      ...
      if ( fabs(T1-T0) < ZERO)
	{
	  ...
	  T1 = WDB[nuc + NUCLIDE_TMS_MAX_TEMP];
	} 
...
#pragma omp parallel for private(n, i, f, maj0) 
	...
	for (n = 0; n < ne; n++)
	  {
	    ...
	    /* Get elastic xs for subtraction in case of OTF S(a,b) */
	...
	if( (sab = (long)RDB[nuc + NUCLIDE_PTR_SAB]) > VALID_PTR){
	  ...
	  /* Alloc & init temporary array for majorant */
