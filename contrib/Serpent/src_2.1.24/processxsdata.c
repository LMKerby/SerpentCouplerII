...
void ProcessXSData()
{
  ...
  double *XSS, *xs, *xs0, *E0, *tmp, Emin, Emax, mem;
  ...
  fprintf(out, "Processing cross sections and ENDF reaction laws...\n\n");
  ...
  while (nuc > VALID_PTR)
      ...
      else if ((long)RDB[nuc + NUCLIDE_TYPE] == NUCLIDE_TYPE_DOSIMETRY)
	{
	  ...
	  while (rea > VALID_PTR)
		...
		Die (FUNCTION_NAME, "Invalid reaction type: %s %ld %ld\n",
	      ...
	      /* Add extra points just below and above boundaries (en tiedä */
	      /* onko tällä mitään väliä) */
	...
	case NUCLIDE_TYPE_DECAY:
	  {		
		...
		*/
		...
		WDB[rea + REACTION_PTR_EGRID] = (long)erg;
		...
		else if ((mt == 102) && ((long)RDB[rea + REACTION_RFS] == 1))
		  WDB[nuc + NUCLIDE_PTR_NGAMMAXS_ISO] = (double)rea;
	  ...
	  {		
		...
		if (mt == 301)
		  InterpolateData(&RDB[pte], xs, ne, &XSS[L0], 
				  &XSS[L1], NES, 2, NULL, NULL);
		else
		  InterpolateData(&RDB[pte], xs, ne, &XSS[L0], 
				  &XSS[L1], NES, 1, NULL, NULL);
      ...
      if ((long)RDB[nuc + NUCLIDE_TYPE] != NUCLIDE_TYPE_DOSIMETRY)
	{
	  ...
	  /* Process energy-dependent isomeric branching ratios */
