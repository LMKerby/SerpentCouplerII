...
void CheckDuplicates()
{
  ...
  fprintf(out, "Checking duplicate input definitions...\n");  
  ...
  /***** Materials in temperature feedbacks **********************************/

  /* Loop over temperature feedbacks */

  loc0 = RDB[DATA_PTR_TFB0];		
  while (loc0 > VALID_PTR)
    {
      /* First loop */

      loc1 = RDB[loc0 + TFB_PTR_REG_LIST];
      while (loc1 > VALID_PTR)
	{
	  /* Second loop */
	  ...
	  while (ptr > VALID_PTR)
	    {
	      ...
	      if (CompareStr(loc1 + TFB_REG_PTR_MAT, ptr + TFB_REG_PTR_MAT))
		Error(loc0, "Duplicate material %s in list",
		      GetText(loc1 + TFB_REG_PTR_MAT));
  ...
  while (loc0 > VALID_PTR)
      ...
      while (ptr > VALID_PTR)
	{
	  ...
	  if (CompareStr(loc0 + DIV_PTR_MAT, ptr + DIV_PTR_MAT))
	    Error(loc0, "Duplicate definition %ld in file \"%s\"",
		  (long)RDB[ptr + PARAM_LINE], 
		  GetText(ptr + PARAM_PTR_FNAME));
