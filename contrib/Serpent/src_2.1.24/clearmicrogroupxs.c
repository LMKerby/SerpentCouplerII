...
{
  long ptr, nmg, gcu, adf, ppw, alb, n, m, i;
	  ...
	  if ((long)RDB[DATA_OPTI_POISON_CALC] == YES)
	    {
	      ...
	      CheckPointer(FUNCTION_NAME, "(ptr34)", RES2_ARRAY, ptr);
	      memset(&RES2[ptr], 0.0, nmg*sizeof(double));
	      ...
	      CheckPointer(FUNCTION_NAME, "(ptr36)", RES2_ARRAY, ptr);
	      memset(&RES2[ptr], 0.0, nmg*sizeof(double));
