...
{
  long loc0, loc1, nf, ptr, pts, n, msh, sld, cell;
  double x1, y1, z1, x2, y2, z2, x3, y3, z3, l, xmin, xmax, ymin, ymax;
  double zmin, zmax, dx, dy, dz, lims[6], mem, V;
  ...
  while (loc0 > VALID_PTR)
      ...
      while (loc1 > VALID_PTR)
	{
	  ...
	  /* Check fill pointer and fail */

	  if ((long)RDB[loc1 + STL_BODY_PTR_FILL] > VALID_PTR)
	    Die(FUNCTION_NAME, 
		"Filled STL solids not yet supported (zonecount.c fails)");
      ...
      while (sld > VALID_PTR)
	{
	  ...
	  if (loc1 > VALID_PTR)
	    ...
	    Error(loc0, "Solid body \"%s\" not defined",
      ...
      fprintf(out, "Processing STL geometry in universe %s:\n",
	...
	Error(loc0, "Structure is not 3D");
      ...
      loc1 = (long)RDB[loc0 + STL_PTR_SOLIDS];
      while (loc1 > VALID_PTR)
	{
	  /*******************************************************************/
	  ...
	  for (n = 0; n < nf; n++)
		...
		Error(loc0, 
		      "Degenerate facet, try decreasing merge radius");
	      else
		Error(loc0, "Degenerate facet, unable to proceed");
      ...
      while (loc1 > VALID_PTR)
	{
	  ...
	  else if ((long)RDB[loc1 + STL_BODY_PTR_FILL] > VALID_PTR)
	    fprintf(out, " - Filled with universe: \"%s\"\n", 
		    GetText(loc1 + STL_BODY_PTR_FILL));
	  else
	    Die(FUNCTION_NAME, "Material and fill pointers null");

	  fprintf(out, " - %ld components\n", 
		  (long)RDB[loc1 + STL_BODY_N_PARTS]);
	  fprintf(out, " - %ld triangular facets\n", 
		  (long)RDB[loc1 + STL_BODY_N_FACETS]);
	  fprintf(out, " - %ld (unique) points\n", 
	...
	TestSTLSolids(loc0, 100000, 10);
      ...
      V = FillSTLMesh(loc0, msh, xmin, ymin, zmin);      
	...
	fprintf(out, " %ld", n);
      ...
      fprintf(out, " - %1.1f%% of volume consists of pre-assigned data\n",
	      100.0*V/((xmax - xmin)*(ymax - ymin)*(zmax - zmin)));

      mem = (double)MemCount();

      if (mem < MEGA)
	fprintf(out, " - %1.2f kb of memory allocated for search mesh\n\n", 
		mem/KILO);
      else if (mem < GIGA)
	fprintf(out, " - %1.2f Mb of memory allocated for search mesh\n\n", 
		mem/MEGA);
      else
	fprintf(out, " - %1.2f Gb of memory allocated for search mesh\n\n", 
		    mem/GIGA);
