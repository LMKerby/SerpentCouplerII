...
/*             rajojen korostuksen.                                          */
/*                                                                           */
/*           - Tää vuotaa muistia jostain? (Saattoi johtua noista väri-      */
/*             määrittelyistä, jotka korjattiin 10.5.2012 / 2.1.6)           */
/*                                                                           */
...
void GeometryPlotter(long ini)
{
  ...
  long n, m, gpl, xp, yp, cell, mat, nerr, old, mode, id, ptr, i;
  long R[COLOURS], G[COLOURS], B[COLOURS], count, nplot, ext;
  long ncol, nifc, nf, nftot, fiss;
  long **matrix;
  ...
  /* Reduce number of colors if IFC or source point animation is in use */
  ...
  if (i == YES)
    Note(0, "Number of pre-assigned material colors exceeds maximum (252)");
  ...
  /***** Put colours for source point animation ******************************/
	  ...
	  if ((long)RDB[DATA_SOURCE_PT_ANIM] == YES)
		...
		for (n = 0; n < xp; n++)
		  for (m = 0; m < yp; m++)
			...
			matrix[n][m] = (long)(f*((double)ncol - 1.0)) + ncol;
		      }
	    }
	  ...
	  if ((long)RDB[DATA_TRACK_PLOTTER_HIS] > 0)
	    {
	      ...
	      fiss = PlotTracks(gpl, im, nf, xmin, xmax, ymin, ymax, 
				zmin, zmax, xp, yp);
