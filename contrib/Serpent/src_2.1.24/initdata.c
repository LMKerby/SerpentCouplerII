...
void InitData()
{
  ...
  /* Allocate memory for random number seed vector */
 ...
 /* Maximum fraction of system memory to use */
  ...
  WDB[DATA_PHOTON_EMAX] = 100;

  /* Set boundary conditions and albedos */
  ...
  WDB[DATA_EVENT_RECORD_MODE] = (double)EVENT_MODE_ALL;

#ifdef OLD_IFP

  WDB[DATA_EVENT_RECORD_MODE] = (double)EVENT_MODE_NONE;
  ...
  /* Calculation of line numbers in input files */
  ...
  /* Reset coefficient calculation index */
  ...
  /* Direct reaction sampling lists (ei näyttäisi olevan väliä) */

  WDB[DATA_OPTI_DIRECT_SAMPLE_LIST] = (double)NO;
