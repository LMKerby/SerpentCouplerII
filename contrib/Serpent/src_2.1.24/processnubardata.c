...
{
  long nuc, ace, ptr, mt, loc0, loc1, n, i;
  ...
  if ((NG = NXS[7]) > MAX_PRECURSOR_GROUPS)
  ...
  else if (NG != (long)RDB[DATA_PRECURSOR_GROUPS])
    {
      ...
      Warn(FUNCTION_NAME, 
	   "Delayed neutron data discarded for %s (mismatch in groups)",
	   GetText(nuc + NUCLIDE_PTR_NAME));
  ...
  for (n = 0; n < NG; n++)
    {
      ...
      lambda = XSS[L]/SHAKE;
      CheckValue(FUNCTION_NAME, "lambda", "", lambda, 1E-2, 11.0);
  ...
  for (n = 0; n < NP; n++)
    {
      ...
      CheckValue(FUNCTION_NAME, "dnu", "", RDB[loc1 + n], 0.0, 0.1);
    }
