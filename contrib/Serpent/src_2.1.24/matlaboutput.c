...
void MatlabOutput()
{
  long gcu, ptr, adf, ppw, alb, ng, nd, n, uni, nb, np, ns, nc;
  ...
  if ((long)RDB[DATA_COEF_CALC_IDX] > 0)
	...
	return;
      else if ((long)RDB[DATA_COEF_CALC_IDX] == 1)
	fp = fopen(outfile, "w");
      else
	fp = fopen(outfile, "a");
    }
  ...
  do
	...
	fprintf(fp, "TFB_SOLUTION_TIME         (idx, [1:  2])  = %12.5E ;\n", 
		TimerVal(TIMER_TFB)/60.0);
	...
	fprintf(fp, "OMP_AMDAHL_MAX            (idx, 1)        = %1.2f ;\n",
      ...
      */
      ...
      PrintValues(fp, NULL, RES_CPU_TIME_PROFILE, 2, -1, -1, 0, 0);
	  ...
	  */
	  ...
	  if (gcu > VALID_PTR)
	      ...
	      if ((1 == 2) && ((long)RDB[DATA_B1_CALC] == YES))
		{
	      ...
	      ng = (long)RDB[ptr + ENERGY_GRID_NE];
	      ...
	      fprintf(fp, "MICRO_E                   (idx, [1: %3ld]) = [ ", ng);
	      for (n = ng - 1; n > -1; n--)
		fprintf(fp, "%12.5E ", RDB[ptr + n]);
