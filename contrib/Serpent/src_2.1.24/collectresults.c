...
void CollectResults()
{
  ...
  long uni, tfb, nst, reg, tb, nr, k, pts, nc, np, nseg, ma0, ms0, g0, g1;
  ...
  val = BufVal(ptr, 0);
  AddStat(val/(double)CLOCKS_PER_SEC, ptr, 0);

  val = BufVal(ptr, 1);
  AddStat(val/(double)CLOCKS_PER_SEC, ptr, 1);
