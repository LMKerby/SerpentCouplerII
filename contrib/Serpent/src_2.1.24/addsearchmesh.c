		   ...
		   double ymin, double ymax, double zmin, double zmax)
{
  ...
  long loc2, loc3, szp, dtype, ns, pts;
  double dx, dy, dz, lims[6], xmin0, xmax0, ymin0, ymax0, zmin0, zmax0;
  double xmin1, xmax1, ymin1, ymax1, zmin1, zmax1, x, y, z, x1, y1, z1;
  double x2, y2, z2, x3, y3, z3, A, B, C, D, d;
	  ...
	  if ((long)RDB[msh + MESH_CONTENT_DATA_TYPE] == MESH_CONTENT_DATA_STL)
	    {
	      ...
	      x = xmin0 + ((double)i + 0.5)*dx;
	      y = ymin0 + ((double)j + 0.5)*dy;
	      z = zmin0 + ((double)k + 0.5)*dz;
	      
	      CheckValue(FUNCTION_NAME, "x", "", x, xmin0 + 0.499999*dx,
			 xmax0 - 0.499999*dx);
	      CheckValue(FUNCTION_NAME, "y", "", y, ymin0 + 0.499999*dy,
			 ymax0 - 0.499999*dy);
	      CheckValue(FUNCTION_NAME, "z", "", z, zmin0 + 0.499999*dz,
			 zmax0 - 0.499999*dz);
	      ...
	      while (loc2 > VALID_PTR)
		{
		  ...
		  if ((long)RDB[msh + MESH_CONTENT_DATA_TYPE] ==
		      MESH_CONTENT_DATA_TET)
		    {
		      ...
		      xmin1 = RDB[ptr + IFC_TET_MSH_XMIN];
		      xmax1 = RDB[ptr + IFC_TET_MSH_XMAX];
		      ymin1 = RDB[ptr + IFC_TET_MSH_YMIN];
		      ymax1 = RDB[ptr + IFC_TET_MSH_YMAX];
		      zmin1 = RDB[ptr + IFC_TET_MSH_ZMIN];
		      zmax1 = RDB[ptr + IFC_TET_MSH_ZMAX];
		    }
