	  ...
	  if ((div = (long)RDB[mat + MATERIAL_PTR_DIV]) > VALID_PTR)
	    {
	      ...
	      n1 = (long)RDB[div + DIV_NAX];
	      n2 = (long)RDB[div + DIV_NRAD];
	      n3 = (long)RDB[div + DIV_NSEG];
	      
	      /* Check division */
	      
	      if ((n0 > 0) && (n0*n1*n2*n3 > 1))
		{
		  /* Print */
		  ...
		  if (n1 > 1)
		    fprintf(out, " - %ld axial sub-regions\n", n1);
