...
void ProcessSymmetries()
{
  ...
  while (sym > VALID_PTR)
	...
	Error(0, "Invalid boundary condition \"%s\"", 
      ...
      WDB[sym + SYMMETRY_THETA0] = WDB[sym + SYMMETRY_THETA0]*PI/180.0;
      WDB[sym + SYMMETRY_ROT] = WDB[sym + SYMMETRY_ROT]*PI/180.0;
