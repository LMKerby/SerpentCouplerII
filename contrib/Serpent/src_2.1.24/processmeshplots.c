...
void ProcessMeshPlots()
{
  ...
  while(mpl > VALID_PTR)
      ...
      else if (ax == 4)
	{
      ...
      if (ax < 4)
	ptr = CreateMesh(MESH_TYPE_CARTESIAN, MESH_CONTENT_DATA, -1, nx, ny, 1,
			 lims, -1);
      else
	ptr = CreateMesh(MESH_TYPE_CYLINDRICAL, MESH_CONTENT_DATA, -1, nx, 1, 
			 ny, lims, -1);
      ...
      if (div == YES)
	{
	  ...
	  ptr = CreateMesh(MESH_TYPE_CARTESIAN, MESH_CONTENT_DATA, -1, nx, ny, 
			   1, lims, -1);
