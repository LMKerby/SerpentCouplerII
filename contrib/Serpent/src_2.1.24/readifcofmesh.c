...
void ReadIFCOFMesh(long loc0, long update)
{
  ...
  long nbhr, divide, hexfaces, hexcells, dtype, ttype, dim[7], *matarr, nmat;
  ...
  char matfile[MAX_STR], mname[MAX_STR];
  FILE *fp, *fp1;
  ...
  if(!update)
      ...
      for (n = 0; n < nf; n++)
	{
	  ...
	  /* Check if this face has four points */
  ...
  if(!update)
      ...
      for (n = 0; n < nc; n++)
	{
	  ...
	  /* Check if this cell has six faces */
