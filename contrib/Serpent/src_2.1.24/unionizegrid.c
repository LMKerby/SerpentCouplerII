...
void UnionizeGrid()
{
  ...
  long ng;
  double *XSS, *Eg, *Ena, *Eni, *tmp, tol, Emin, Emax, mem;
  ...
  if ((long)RDB[DATA_OPTI_UNIONIZE_GRID] == NO)
    {
      fprintf(out, "Energy grid unionization not performed. Minimum and\n");
      fprintf(out, "maximum energy set to %1.2E and %1.1f MeV.\n\n",
  ...
  fprintf(out, "Generating unionize energy grids...\n");
  fprintf(out, "\nAdding points:\n\n");
  ...
  while (nuc > VALID_PTR)
	  ...
	  ((ace = (long)RDB[nuc + NUCLIDE_PTR_ACE]) > VALID_PTR))
	{
	  ...
	  fprintf(out, "%10s -- Points added in neutron grid: %ld\n", 
  ...
  while (nuc > VALID_PTR)
	  ...
	  ((ace = (long)RDB[nuc + NUCLIDE_PTR_ACE]) > VALID_PTR))
	{
	  ...
	  fprintf(out, "%10s -- Points added in photon grid: %ld\n", 
		  GetText(nuc + NUCLIDE_PTR_NAME), tot);
	}
  ...
  if ((long)RDB[DATA_NEUTRON_TRANSPORT_MODE] == YES)
    {
      ...
      if (tol > 0.0)
	{
	  fprintf(out, "Performing grid thinning for neutron grid:\n\n");
	  fprintf(out, " - %ld total grid points in unionized grid, with\n", 
		  na);
	  fprintf(out, "   %ld local minima and maxima.\n\n", ni);
	}
      else
	fprintf(out, "Generating unionized energy grid:\n\n");
	...
	if (RDB[ptr + n] <= RDB[ptr + n - 1])
      ...
      if (tol > 0.0)
	fprintf(out, " - Thinning performed with %1.2E fractional tolerance\n",
		tol);
      else
	fprintf(out, " - Unionization performed without grid thinning\n");
      
      fprintf(out, "   between %1.2E and %1.1f MeV.\n\n", Emin, Emax);     
      
      fprintf(out, " - Final neutron grid size: %ld points.\n\n", np);
  ...
  else if ((long)RDB[DATA_PHOTON_TRANSPORT_MODE] == YES)
    {
      ...
      fprintf(out, "Only photon grid generated:\n\n");
    }
  ...
  if (ng > 0)
    fprintf(out, " - Final photon grid size: %ld points.\n\n", ng);

  if ((mem = (RDB[DATA_TOTAL_BYTES] - mem)/KILO) < KILO)
    fprintf(out, " - %1.2f kb of memory allocated for grid data\n\n", 
	    mem);
  else
    fprintf(out, " - %1.2f Mb of memory allocated for grid data\n\n", 
	    mem/KILO);
