...
{
  long sz, n, nnuc, zai, mat, iso, nuc, *ptr, ok;
  ...
  if (((pt > 0.0) && (fabs(closest/pt - 1.0) > 0.001)) ||
      ...
      while ((sz = fread(&n, sizeof(long), 1, fp)) > 0)
	{
	  ...
	  if ((sz = fread(tmpstr, sizeof(char), n, fp)) == 0)
	    Die(FUNCTION_NAME, "Error in restart file 11");
	  if ((sz = fread(&bu, sizeof(double), 1, fp)) == 0)
	    Die(FUNCTION_NAME, "Error in restart file 12");
	  if ((sz = fread(&days, sizeof(double), 1, fp)) == 0)
	    Die(FUNCTION_NAME, "Error in restart file 13");
	  if ((sz = fread(&nnuc, sizeof(long), 1, fp)) == 0)
	    Die(FUNCTION_NAME, "Error in restart file 14");
	  if ((sz = fread(&adens, sizeof(double), 1, fp)) == 0)
	    Die(FUNCTION_NAME, "Error in restart file 15");
	  if ((sz = fread(&mdens, sizeof(double), 1, fp)) == 0)
	    Die(FUNCTION_NAME, "Error in restart file 16");
	  if ((sz = fread(&mbu, sizeof(double), 1, fp)) == 0)
	    Die(FUNCTION_NAME, "Error in restart file 17");
	  ...
	  if (days != d0)
	    {
	      ...
	      fprintf(stdout, "BU = %7.3f MWd/kgu, time = %1.5E days", 
		      bu, days);
  ...
  else if ((pt > 0.0) && (fabs(closest/pt - 1.0) > 1E-9))
	  ...
	  if (mode == RESTART_REPLACE)
	      ...
	      for (n = 0; n < nnuc; n++)
		{
		  ...
		  if ((sz = fread(&zai, sizeof(long), 1, fp)) == 0)
		    Error(0, "Error in restart file 8");
		  
		  if ((sz = fread(&adens, sizeof(double), 1, fp)) == 0)
		    Error(0, "Error in restart file 9");
