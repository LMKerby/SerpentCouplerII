...
void ProcessEDistributions(long rea, long prec)
{
  ...
  if ((mt == 1002) || (mt == 1004))
      ...
      if (mt == 1004)
	{
	  ...
	  l0 = ReallocMem(DATA_ARRAY, NES*NXS[3]*(NXS[2] + 2) + 4 + NES + 1);
	  ...
	  for (n = 0; n < NES; n++)
	      ...
	      for(i = 0; i < NXS[3]; i++)
		{
		  ...
		  CheckValue(FUNCTION_NAME, "E1", " (sab)", E, 1E-12, 1E-3);
  ...
  while (L0 > 0)
	...
	Die(FUNCTION_NAME, "Unable to process E interpolation (mt %ld)", mt);
	...
	default:
	  {
	    ...
	    Die(FUNCTION_NAME, "Unsupported law %ld (mt %ld)", LAW, mt);
