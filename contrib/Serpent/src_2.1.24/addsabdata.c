...
void AddSabData()
{
  ...
  while (mat > VALID_PTR)
      ...
      while (loc0 > VALID_PTR)
	{
	  ...
	  /* Check if S(a,b) nuclide already created and skip */
	  ...
	  if ((long)RDB[loc1 + THERM_INTERP_MODE] == THERM_INTERP_OTF)
		...
		Error(loc1, "Minimum interpolation T below smallest T found in S(a,b) data (%.2f < %.2f)", 
	      ...
	      WDB[new + NUCLIDE_PTR_SAB] = sab;
	      WDB[loc1 + THERM_PTR_SAB] = sab;
