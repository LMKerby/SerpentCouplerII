...
void CheckNuclideData()
{
  ...
  fprintf(out, "Checking data and printing output...\n");
  ...
  while ((nuc = ListPtr((long)RDB[DATA_PTR_NUC0], n++)) > VALID_PTR)
	...
	Warn(FUNCTION_NAME, "Nuclide %s has no type",
      ...
      CheckValue(FUNCTION_NAME, "A", "", RDB[nuc + NUCLIDE_A], 0, 260);
	...
	Warn(FUNCTION_NAME, "Mismatch in ZA (%s)", 
      ...
      CheckValue(FUNCTION_NAME,"AW", "", RDB[nuc + NUCLIDE_AW], 0.9, 260.0);
  ...
  if (1 != 2)
	...
	fprintf(fp, " - All reaction linked to target\n\n");
      ...
      fprintf(fp, "            E - nuclide cross sections are adjusted by TMS\n");
      ...
      while ((nuc = ListPtr((long)RDB[DATA_PTR_NUC0], n++)) > VALID_PTR)
	{
	  ...
	  if ((ace = RDB[nuc + NUCLIDE_PTR_DECAY_ACE]) < VALID_PTR)
	    fprintf(fp, "        N/A  ");
  ...
  if (1 != 2)
      ...
      while ((nuc = ListPtr((long)RDB[DATA_PTR_NUC0], n++)) > VALID_PTR)
	{
	  ...
	  if ((long)RDB[nuc + NUCLIDE_TYPE_FLAGS] & NUCLIDE_FLAG_DEP)
		...
		fprintf(fp, " - Nuclide is not produced in transmustation\n");
	  ...
	  if ((long)RDB[nuc + NUCLIDE_TYPE_FLAGS] 
	      & NUCLIDE_FLAG_SAB_DATA)
	    fprintf(fp, " - Nuclide has S(a,b) data\n");
	  else
	    fprintf(fp, " - Nuclide has no S(a,b) data\n");

	  if ((long)RDB[nuc + NUCLIDE_BRA_TYPE] == BRA_TYPE_ENE)
	    fprintf(fp, " - Nuclide has energy-dependent isomeric branching data\n" );
	  else if ((long)RDB[nuc + NUCLIDE_BRA_TYPE] == BRA_TYPE_FIX)
	    fprintf(fp, " - Nuclide has fixed isomeric branching ratios\n" );
	  else
	      ...
	      while (rea > VALID_PTR)
		{
		  ...
		  if ((long)RDB[rea + REACTION_TYPE] == 
		      ...
		      else if ((long)RDB[nuc + NUCLIDE_TYPE_FLAGS] & NUCLIDE_FLAG_DEP)
			{
		      ...
		      if ((long)RDB[rea + REACTION_RTYP2] != 10000)
			fprintf(fp, " + %s", 
				ReactionMT((long)RDB[rea + REACTION_RTYP2]));
		      if ((long)RDB[rea + REACTION_RTYP3] != 10000)
			fprintf(fp, " + %s", 
				ReactionMT((long)RDB[rea + REACTION_RTYP3]));
		      if ((long)RDB[rea + REACTION_RTYP4] != 10000)
			fprintf(fp, " + %s", 
				ReactionMT((long)RDB[rea + REACTION_RTYP4]));
		      if ((long)RDB[rea + REACTION_RTYP5] != 10000)
			fprintf(fp, " + %s", 
				ReactionMT((long)RDB[rea + REACTION_RTYP5]));
	      ...
	      if (ptr == (long)RDB[DATA_PTR_NUCLIDE_LOST])
		{
		  if ((long)RDB[rea + REACTION_TYPE] == REACTION_TYPE_DECAY)
		    fprintf(fp, " %5ld  %10s  RTYP = %3ld           frac = %11.5E  missing target = %7s  :  %s\n", 1 + i++, 
			    GetText(nuc + NUCLIDE_PTR_NAME), mt - 10000, RDB[rea + REACTION_BR],
			    ZAItoIso((long)RDB[rea + REACTION_TGT_ZAI], 1), ReactionMT(mt));
		  else
