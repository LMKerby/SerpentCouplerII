...
void InterpolateNubar(double *nubar, long rea)
{
  ...
  if ((long)RDB[loc0 + NUBAR_DATA_TYPE] == 1)
      ...
      for (n = 0; n < ne; n++)
	{
	  ...
	  CheckValue(FUNCTION_NAME, "nubar", "", nubar[n], 1.5, 10.0);
	}
  ...
  else
    {
      ...
      InterpolateData(E, nubar, ne, &RDB[erg], &RDB[ptr], np, 0, NULL, NULL);
	...
	if (E[n] >= RDB[erg])
	  {
	    ...
	    CheckValue(FUNCTION_NAME, "nubar", "", nubar[n], 0.0, 10.0);
	  }
#endif
  ...
  if ((long)RDB[loc0 + NUBAR_DATA_TYPE] == 1)
  ...
  else
    {
      ...
      InterpolateData(E, dnubar, ne, &RDB[erg], &RDB[ptr], np, 0, NULL, NULL);
