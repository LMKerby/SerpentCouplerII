...
/*           - The new ACE format is described in:                           */
/*                                                                           */
...
{
  long ace, ptr, rea, n, sz, NXS[16], JXS[32], NES, L0, L, NTR, nr, mt, nc;
...
#ifdef DEBUG
		  Warn(FUNCTION_NAME, "MT 5 included (%s), TY = %ld", 
		       GetText(nuc + NUCLIDE_PTR_NAME),
		       (long)RDB[rea + REACTION_TY]);
#endif
		}
