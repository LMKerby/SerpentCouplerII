...
long TrackMode(long part, long mat, double totxs, double majorant, long type,
	       long id)
{
  long ptr;
  double trsh;
  ...
  if ((mat > VALID_PTR) && (totxs/majorant - 1.0 > 1E-6))
    Die(FUNCTION_NAME, "Total exceeds majorant (f = %E, mat = %s)",
	totxs/majorant - 1.0, GetText(mat + MATERIAL_PTR_NAME));
