...
void DivideZone(long mat, long *idx, long uni, long j)
{
  long div, nax, nrad, nseg, ntot, n, ptr, new, out, lst;
  char tmpstr[MAX_STR];
  ...
  if ((long)RDB[div + DIV_SEP] == NO) 
    {
      /* Loop over list is associated with another universe. */
  ...
  for (n = 0; n < ntot; n++)
	...
	RDB[mat + MATERIAL_DIV_N_TOT_ZONES] + 1.0;
      ...
      WDB[new + MATERIAL_DIV_ZONE_IDX] = WDB[mat + MATERIAL_DIV_N_TOT_ZONES];
