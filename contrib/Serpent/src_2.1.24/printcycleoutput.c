...
void PrintCycleOutput()
{
  ...
  if (i > skip)
      ...
      if ((long)RDB[DATA_SIMULATION_MODE] == SIMULATION_MODE_CRIT)
	{
	  ...
	  if(RDB[DATA_RUN_CC] == (double)YES)
	    {
	      fprintf(out, "Simulated population before/after this iteration:\n");
	      fprintf(out, "%6.1f%% / %6.1f%% \n\n",
		    RDB[DATA_SOL_REL_NTOT]/RDB[DATA_SOL_REL_MAXPOP]*100.0,
		    (RDB[DATA_SOL_REL_NCUR] + RDB[DATA_SOL_REL_NTOT])
		      /RDB[DATA_SOL_REL_MAXPOP]*100.0);
	    }
	}
      else
	{
  ...
  if ((i == cycles + skip) && ((long)RDB[DATA_SIMULATION_MODE] != SIMULATION_MODE_DYN))
