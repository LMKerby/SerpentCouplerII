...
void ScoreMesh(long part, long mat, double flx0, double dE, double x0, 
	       ...
	       long id)
{
  long mpl, ax, mode, ptr, rea, det, type;
  double x, y, z, xs, H, f, T, majorant, V, flx;
  ...
  while(mpl > VALID_PTR)
      ...
      else if ((mode == MPL_TYPE_DET) || (mode == MPL_TYPE_DET_IMP))
	{
	  ...
	  else if (DetBin(det, mat, x0, y0, z0, E, t, 0, id) > -1)
	    f = DetResponse(det, ptr, mat, E, g, id);
	  else
	    f = 0.0;
	  ...
	  if (mode == MPL_TYPE_DET)
	      ...
	      if (f != 0.0)
		{
		  ...
		  CheckPointer(FUNCTION_NAME, "(hst0)", DATA_ARRAY, hst0);
