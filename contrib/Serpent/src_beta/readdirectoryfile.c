...
void ReadDirectoryFile()
{
  ...
  fprintf(out, "Reading ACE directory files...\n");
	...
	{	  
	  ...
	  if ((type < 1) || (type > 7) || (type == 4) || (type == 6))
	      ...
	      else
		{
	      ...
	      ACE[ace + ACE_PTR_ALIAS] = PutText(alias);
	      ACE[ace + ACE_PTR_NAME] = PutText(name);
	      ACE[ace + ACE_PTR_LIB_ID] = PutText(&name[strlen(name) - 3]);
	      ...
	      ACE[ace + ACE_ZAI] = (double)I + 10.0*ZA;
	      ...
	      ACE[ace + ACE_PTR_FILE] = PutText(path);
	    }
