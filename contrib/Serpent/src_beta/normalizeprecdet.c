...
void NormalizePrecDet()
{
...
#ifdef DNPRINT
  fprintf(out, "normalizeprecdet.c -->\n");  
#endif
...
#ifdef DNPRINT
  fprintf(out, "Should emit %E delayed neutrons in total\n", Wall);
#endif

  /* Loop over precursor list to calculate total weight to be emitted from them */
  /* Maybe put this if-block in a separate suborutine? */
...
#ifdef DNPRINT
      fprintf(out, "Would emit %E delayed neutrons from list\n", Wemit_list);
      fprintf(out, "List weights have to be scaled by %E\n", Wall/Wemit_list);
#endif
  ...
  toemit = (long)round(Wall/(Wlive + Wall)*RDB[DATA_SRC_POP]);

  /* Calculate number of live neutrons to get from srcfile */

  tolive = (long)round(Wlive/(Wlive + Wall)*RDB[DATA_SRC_POP]);
  ...
  if (tolive > 0)
    {
      ...
      WDB[DATA_NORM_COEF_N] = Wlive / tolive * (double)mpitasks;

#else

      /* Set normalization */

      WDB[DATA_NORM_COEF_N] = Wlive / tolive;
#endif
    }
  else
    {
      ...
      WDB[DATA_NORM_COEF_N] = Wemit / toemit * (double)mpitasks;
      ...
      WDB[DATA_NORM_COEF_N] = Wemit / toemit;
  ...
  if (RDB[DATA_PRECURSOR_TRANSPORT_MODE] == PREC_MODE_POINT)
    {
      ...
      fprintf(out, "Weight to emit: %f (%E neutrons)\n", 
	      Wemit_list/RDB[DATA_NORM_COEF_N], Wall);
      fprintf(out, "Average neutrons emitted was %E\n", 
	      Wemit_list/Na);
#endif
    }
  else
...
#ifdef DNPRINT
      fprintf(out, "Normalization: 1.0 weight corresponds to %E neutrons\n", 
	RDB[DATA_NORM_COEF_N]);
      fprintf(out, "Weight to emit: %f (%E neutrons)\n", 
...
#ifdef DNPRINT
  fprintf(out, "Weight of live neutrons    %f\n", 1.0);
  fprintf(out, "<-- normalizeprecdet.c\n\n");  
#endif
