...
double DopMicroXS(long mat, long rea, double E, double *Er, double T, long id)
{
  ...
  if((RDB[rea + REACTION_XS_I0] > 0) || (E > RDB[nuc + NUCLIDE_URES_EMIN]) ||
     (E > 1.00))
    {
      *Er=E;
  ...
  if (kT == 0.0)
    *Er=E;       
    ...
    if ((*Er = TestValuePair(nuc + NUCLIDE_PREV_COL_ER, ncol, id)) < 0.0)
      {
	...
	*Er=x2/ar;
	...
	if (RDB[nuc + NUCLIDE_XS_TEMP] == 0.0)
	  {
	    ...
	    StoreValuePair(nuc + NUCLIDE_PREV_COL_COS, ncol, c, id);
	  }
    ...
    StoreValuePair(nuc + NUCLIDE_PREV_COL_ER, ncol, *Er, id);
