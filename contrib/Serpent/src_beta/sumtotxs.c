...
void SumTotXS(long nuc)
{
	  ...
	  while (rea > VALID_PTR)
	      ...
	      else if (mode == NUCLIDE_PTR_PHOTPRODXS)
		{
		  ...
		  if ((long)RDB[rea + REACTION_TYPE] == REACTION_TYPE_SPECIAL)
		    if ((long)RDB[rea + REACTION_MT] > 3000)
		      if (rea != rea0)
			add = YES;
		}
	      else
  ...
  /***** Multiply heating values with total xs *******************************/
  ...
  if ((long)RDB[nuc + NUCLIDE_TYPE_FLAGS] & NUCLIDE_FLAG_SAB_DATA)
    {
      ...
      if (WDB[ptr + NUCLIDE_PTR_HEATPRODXS] < VALID_PTR)
  ...
  if ((long)RDB[nuc + NUCLIDE_PTR_HEATPRODXS] > VALID_PTR)
	...
	Die(FUNCTION_NAME, "XS does not start from zero");
      ...
      InterpolateData(&RDB[ptr], xs, sz, E0, xs0, ne, 0, NULL, NULL);      

      /* Pointer to heat production cross section */
