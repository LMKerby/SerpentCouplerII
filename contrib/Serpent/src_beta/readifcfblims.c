...
void ReadIFCFBLims(FILE *fp, long loc1, long update)
{
  ...
  if ((long)RDB[DATA_SIMULATION_MODE] != SIMULATION_MODE_CRIT)
    {
      WDB[loc2 + FUEP_TMIN] = RDB[DATA_DYN_TMIN];
      WDB[loc2 + FUEP_TMAX] = RDB[DATA_DYN_TMAX];
      WDB[loc2 + FUEP_NT] = (double)(RDB[DATA_DYN_NB]*nnt);
    }
  else
    {
      WDB[loc2 + FUEP_TMIN] = -INFTY/10.0;
      WDB[loc2 + FUEP_TMAX] =  INFTY/10.0;
      WDB[loc2 + FUEP_NT] = (double)1;
    }
