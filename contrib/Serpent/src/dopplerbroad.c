...
void DopplerBroad()
{
  ...
  fprintf(out, "Running Doppler-broadening preprocessor:\n\n");
  ...
  while (nuc > VALID_PTR)
      ...
      if ((long)RDB[nuc + NUCLIDE_TYPE] == NUCLIDE_TYPE_TRANSPORT)
	{
	  ...
	  /* TODO: lis‰‰ johonkin tarkistus materiaalin l‰mpˆtilasta */
	    ...
	    Die(FUNCTION_NAME, 
		"Doppler-broadened temperature below original (%s)",
	  ...
	  for (nr = 0; nr < NTR; nr++)
		...
		mik‰li todn‰ktaulut lˆytyv‰t sek‰ 
		lasketaan integrointirajat */
		...
		if((JXS[22] > 0) && (XSS[JXS[22]+6-1] > 1E-8)){
		  ...
		  for(i=0; i<10; i++){
		    if(XSS[L0+UNRES+(long)floor((uplimit-UNRES)/2.0)]<XSS[JXS[22]+6-1]){
		      UNRES=(long)floor((uplimit-UNRES)/2.0)+UNRES;
		    }
		    else {
		      uplimit=(long)floor((uplimit-UNRES)/2.0)+UNRES;
		    }
		  }
		...
		/* Jos prob. tableja ei ole, tyydyt‰‰n integroimaan koko 
		   energia-alueen yli. T‰m‰ aiheuttaa l‰hes 
		   merkityksettˆm‰n lis‰n laskenta-aikaan ja
		   virheen vaikutusalaan unresolved-rajalle. */
		else{
		...
		*/
		...
		/* Limits-vektorista ainoastaan soveltuvat osat
		   BCS-funktiolle (NXS[2]-NES l‰htien)
		   NXS[2] sis‰lt‰‰ energiapisteiden m‰‰r‰n energiagridiss‰,
		   kun taas NES sis‰lt‰‰ reaktion energiapisteiden m‰‰r‰n.
		   Viimeinen piste j‰tet‰‰n aina levent‰m‰tt‰ (3. argumentissa -1) */
		
		BroadCrossSection(L0, L1, UNRES-(NXS[2]-NES)-1, T, Td, 
				  awr, XSS, &limits[(NXS[2]-NES)*3], NXS[2]-NES, NXS[2]);
