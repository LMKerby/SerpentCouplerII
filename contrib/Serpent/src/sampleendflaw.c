		   ...
		   long id)
{
  ...
  switch(law)
	...
	*Eout = d0 + (rnd1*(nb - 1.0) - k)*(d1 - d0);
	...
	CheckValue(FUNCTION_NAME, "d0 (law 3)", "", d0, -0.5, 35.0);
	...
	for (j = 0; j < 100; j++)
	  {
		...
		if (nd == ne)
		  {
		    /* Print warning (voi olla tarpeeton) */
		    
		    Warn(FUNCTION_NAME, "Unable to sample discrete line");
		    ...
		    if (j < 2)
		      ...
		      Die(FUNCTION_NAME, "Resample %ld failed", j + 1);
		  }
