...
{
  long L, N, M, INT, IFF, loc0, neg, n, m, i, j, rea, rea0, ptr, urs, mt;
	...
	for (m = 0; m < M; m++)
	  {
	    ...
	    if ((IFF == 0) && ((f < -10.0) || (f > MAX_XS)))
	      Die(FUNCTION_NAME, "Invalid table value %1.5E (%ld IFF = 0)", f);
	    else if (IFF == 1) 
	      {
		if ((f < -1E-1) || (f > 500.0))
		  Die(FUNCTION_NAME, 
		      "Invalid table value %1.5E (IFF = 1)", f);
		else if (f < 0.0)
		  Warn(FUNCTION_NAME, 
		       "Negative table value %1.5E (IFF = 1)", f);
	      }
	...
	for (n = 0; n < N; n++)
	  {
	    ...
	    if ((f = XSS[L + n*6*M + m + (j + 1)*M]) > fmax)
	      fmax = f;

	    /* Compare to value in table m at energy n - 1 */

	    if (n > 0)
	      if ((f = XSS[L + (n - 1)*6*M + m + (j + 1)*M]) > fmax)
		fmax = f;
