...
{
  long mat, iso, ptr, ptf, table, nuc, sab, mix, ndiv, nsub, ntot;
  ...
  while (mat > VALID_PTR)
	...
	{      
	  ...
	  fprintf(fp, " - Atom density %1.4E 1/barn*cm\n", 
		  RDB[mat + MATERIAL_ADENS]);
	  fprintf(fp, " - Mass density %1.4E g/cm3\n", 
		  RDB[mat + MATERIAL_MDENS]);

	  fprintf(fp, " - Volume %1.4E cm3\n", RDB[mat + MATERIAL_VOLUME]);

	  fprintf(fp, " - Mass %1.4E g\n", RDB[mat + MATERIAL_MASS]);

	  fprintf(fp, " - Photon emission rate %1.4E 1/s\n", 
	  ...
	  if ((mix = (long)RDB[mat + MATERIAL_PTR_MIX]) > VALID_PTR)
	    {
	      ...
	      fprintf(fp, "-------------------------------------------------------------------\n");
	      fprintf(fp, "Material             v. frac      m. frac       volume         mass\n"); 
	      fprintf(fp, "-------------------------------------------------------------------\n");
